...
**Date:** 2026-09-02 to 2026-09-03 · **Task:** T-572 · **Status:** delivered; dataset at `D:\claude\colmap-gh\hall\` (46 GB of frames, a 14 GB database, 17 GB of models), trainer package at `D:\claude\colmap-gh\hall-t502\`, both preflight-proven · **Instruction:** "run the bridge over the whole Grand Hall at a higher budget and hand the refined dataset to the T-502 trainer" · **Tool:** `tools/xgrids-xbag/xbag_colmap.py` (checkpoint 5db7aebd, 65 unit tests) · **Predecessor:** `xbag-colmap-zone-2026-09-02.md` (T-571, the south-west quarter)
...
This machine's commit limit is about 50 GB, of which other software holds about 30 GB; a torch process commits 9 GB on its own. Four decoders, a package builder, a Ceres adjustment and a mesh check together exhausted it twice (`Cannot allocate memory` from PyAV, `_ArrayMemoryError` from numpy, a killed worker in the package builder). The pipeline was rewritten to run one heavy stage at a time, which is why the whole hall takes the better part of a day on this laptop: the second, correct pass alone ran from 03:13 to 08:04 (re-verification 17 min, scoring 52 min, strict triangulation 35 min, loose 45 min, refinement 9 min, re-triangulation 39 min, refinement 7 min, checks 12 min, package 50 min), on top of the 41 minutes of GPU features, 43 minutes of pinhole matching, 2.7 hours of fisheye SIFT and 3.5 hours of fisheye matching before it, and the 1.5 hour preflight after it. The same work on a rented GPU box with a CUDA COLMAP is about an hour; it needs the RunPod key that does not exist here. A larger pagefile on D: (a system setting the owner must make) would let the GPU and CPU stages overlap here and roughly halve the wall-clock.
