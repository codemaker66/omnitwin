...
Unthrottled, paging fetched nearly the whole tree, and the tree format is 3.5 times the tile's bytes; the compact `--csplat` encoding produced 378 MB against 381, so the encoding is not the lever. The model that followed (trees win only above about 250 Mbps) was then tested with the harness's new throttle, and the model was wrong, because a bandwidth cap is exactly when a pager pages:

| label, 100 Mbps emulated | what loads | time to complete load | wire at load | wire after the drags | heap MB |
|---|---|---|---|---|---|
| t100-tiles | the twelve tiles, no tree | 13.4 s | 101.9 MB | 101.9 MB | 435 |
| t100-browser-tree | the tiles, tree built in the browser | 19.4 s | 101.9 MB | 101.9 MB | 634 |
| t100-prebuilt-trees | twelve prebuilt trees, paged | 2.5 s | 88.2 MB | 155.5 MB | 1004 |

Under the cap Spark fetched the pages the first view needed and streamed the rest as the camera moved. Two more runs with readback shots say what that first view is (`t100-trees-shot8.png`, `t100-trees-shot25.png`): at 8 s the room is there but soft, the name boards a blur and the mural smeared, with 82 MB fetched; at 25 s it is complete and legible, with 284 MB fetched (314 MB after the drags). So the prebuilt trees are "appear in 2.5 s and sharpen over half a minute" and the tiles are "nothing for 13 s, then crisp", at three times the visitor's data (R2 itself charges nothing for egress) and a paged pool that sat at 1 GB of heap here, which a tier can bound through the renderer's `maxPagedSplats` (Spark's defaults: 16.7 M on desktop, 6.3 M on iOS, 8.4 M on other mobile).

That is a taste and bandwidth decision, not an engineering one, and it is left open for the owner with both pictures in the folder. Tonight's default stays with the tiles on the high tier (crisp when it arrives, 102 MB, 435 MB heap, 176 fps). If the sharpening view is preferred, the order is: cap the pool per tier, publish the trees to R2 first (2.0 GB), then set `preferTrees` on; and a connection-aware switch (measure the first tile's download rate, prefer trees above a threshold) would give both behaviours to the visitors who benefit from each. The decision, from these numbers: the high tier runs the tiles as they are, no tree (176 fps at full detail, 4.4 s load, 434 MB heap, all measured); the three weaker tiers keep the tree on and build it in the browser from the tiles, which keeps the wire at 102 MB; the prebuilt trees stay in the manifest and on disk as an opt-in (`?splat=trees:on`) for fast connections and for the harness, and are not published to R2 tonight. The cheaper protection for weak devices, not yet built, is the vendor's own coarser levels that the staged bundle already holds: serve level four of the Grand Hall's five to a medium device and no tree is needed at all.

## 4b. Production, verified

The same harness pointed at venviewer.com after the deploy (`production-live.json`, `production-live.png`): the Grand Hall walk loads complete (the name boards legible), drags at 165 fps with a p95 of 8.4 ms, 412 MB of heap, no page errors, 102 MB over 12 same-origin tile requests (Vercel rewrites `/splats/` to R2), 31 s to load on this home line, which puts the line at about 26 Mbps down. The embedded Browser pane could not show this on either origin, which is recorded in `.claude/gotchas/browser-pane-splat-streaming.md`: it is not an instrument for splat routes.
