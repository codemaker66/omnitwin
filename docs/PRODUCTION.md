...
## Scary words explained (read this first if you're not technical)

Don't skip this section. These terms come up constantly.

- **API** — the "backend". The program running on Railway that the website talks to.
- **Web / frontend** — the website itself, the thing users see in their browser. Runs on Vercel.
- **DB (database)** — where all customer data lives. Ours is Neon (a Postgres database).
- **DNS** — the phonebook of the internet. Turns `omnitwin.com` into a server address.
- **CNAME / A record / TXT record** — different types of DNS entries. CNAME = "forward this subdomain to that other address." A = "this subdomain lives at this raw IP." TXT = "notes other services can read," mostly used for email verification.
- **CDN** — Content Delivery Network. A copy of your website stored in 100+ cities so it loads fast worldwide. Cloudflare does this free.
- **WAF** — Web Application Firewall. Automatically blocks obvious attacks (SQL injection, bots hammering login). Cloudflare free tier includes basic WAF.
- **R2** — Cloudflare's version of Amazon S3. A bucket for uploaded files (event photos, PDFs). Cheap, fast.
- **TLS / HTTPS / SSL** — the encryption that makes web addresses start with `https://`. Required for all production sites. Vercel and Cloudflare do this automatically.
- **env var (environment variable)** — a secret or config value given to the app at startup. Example: `DATABASE_URL=...`. You paste these into Railway/Vercel dashboards; they never appear in the code.
- **`.env` file** — a file on your own computer listing env vars for local dev. Never committed to git.
- **Webhook** — a URL that external services (Clerk, Stripe, Resend) call when something happens. Example: "a user just signed up → Clerk sends us a webhook → we create their row in our DB."
- **Signing secret** — a shared password that proves a webhook actually came from the service claiming to send it (not a forged request from a stranger).
- **DSN (Sentry)** — a URL that the app uses to send error reports to Sentry.
- **DPA (Data Processing Agreement)** — a legal contract big customers require that says "we'll handle your data properly."
- **SOC 2** — a security certification big customers require. Takes months to get, costs ~£25k+. Not needed until you're selling to Fortune-500 types.
- **SSO (Single Sign-On)** — enterprise customers want employees to log in with their company account (Okta, Google Workspace, etc.) instead of a password. Clerk supports this on paid tiers.
- **MAU** — Monthly Active Users. Most services charge based on this. Clerk's free tier covers 10,000 MAU.
- **MCP** — Model Context Protocol. The way Claude talks to Railway, Vercel, Neon, and GitHub without credentials flowing through chat. Means Claude can "press buttons" on those services for you.
- **pnpm, Vite, Fastify, Drizzle** — developer tools. You don't need to understand them; Claude does.
...
- [Phase 8 — Billing & pricing (how much you charge customers)](#phase-8--billing--pricing-how-much-you-charge-customers)
