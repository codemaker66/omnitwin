...
> **Status, 26 September 2026 — historical, not a live instruction.** Release 1 was not executed as written.
> The lanes were built as pull requests #18 to #30 against `release/r1`, which reached master without them,
> so none of their work reached master or production. They are being integrated into master under T-635,
> lane by lane with fresh verification (see `docs/sessions/2026-09-26.md`), and Blake's later decisions take
> precedence over this document. Do not work from `release/r1` or follow this plan's merge instructions.
