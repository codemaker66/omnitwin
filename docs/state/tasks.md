...
| T-054 | Geass v0+v1+v2 — agentic founder operating system. | deferred | 5 | 3–5 | T-001, T-003 | 2026-04-27 architectural review §818-§921 | Webhook relay (v0) → severity routing and slash commands (v1) → Claude judgment layer (v2) as one coherent build. Discord delivery via private Venviewer Ops server (`#critical`, `#training`, `#deploys`, `#daily-digest`, `#cost-alerts`). Architecture: Cloudflare Worker event hub, Neon Postgres state/event log under `sentinel.*` schema namespace, Claude API for severity classification, Discord webhooks + slash commands for delivery. Trust boundary: read access to system events only, no write access to production data. Reactivation trigger: T-003 status = `done`. |
| T-060 | Error tracking — Sentry on `apps/web` and `packages/api`. | not-started | 4 | 1–2 hr | — | 2026-04-27 audit | Source-map upload via build pipeline. Alerts: any unhandled exception in production = immediate; sustained error rate > 1% over 5 min = warning. Sentry account in EU region for GDPR alignment. |
| T-061 | Uptime monitoring — external availability checks on production endpoints. | not-started | 3 | 0.5 hr | — | 2026-04-27 audit | Better Stack or Cloudflare's free uptime monitoring on `omnitwin-web.vercel.app` and `omnitwinapi-production.up.railway.app` at 1-minute intervals. Alert on 2 consecutive failures. Routing: into Geass once it exists, plain Discord webhook before then. |
| T-062 | Backup verification — confirm Neon restore procedure works end-to-end. | not-started | 4 | 1 hr | — | 2026-04-27 audit | Test restore from a Neon branch to a local Postgres instance. Document at `docs/operations/backup-restore-procedure.md`. Until verified, we have a hope of backups, not a verified backup. Treat as a precondition before any customer data lands. |
| T-063 | Email infrastructure — Venviewer domain in Resend with full DNS auth. | not-started | 4 | 2–3 hr | — | 2026-04-27 audit | Configure `venviewer.com` (or actual customer-facing domain) in Resend with full DKIM, SPF, DMARC. Canonical senders: `noreply@`, `hello@` (or `team@`), `support@`. Configure DNS at the domain registrar layer. Verify with mail-tester.com — must score 10/10, anything less is below the bar. Document sender taxonomy at `docs/operations/email-senders.md`. Email templates for transactional flows tracked separately under T-072. |
| T-064 | Secrets management — single source of truth syncing across all environments. | not-started | 3 | 2–3 hr | — | 2026-04-27 audit | Doppler or Infisical syncing to Vercel env vars, Railway env vars, RunPod env vars, local `.env` files. Goal: a key rotation in one place propagates everywhere within minutes. Evaluate both, pick one, document the choice as ADR. |
| T-065 | Unified logging — cross-system request tracing via Axiom. | not-started | 3 | 2–3 hr | — | 2026-04-27 audit | Axiom (free tier 500 GB/month) for log aggregation across Vercel, Railway, Cloudflare Workers, Neon. Structured logs with request correlation IDs so a single customer request can be traced frontend → API → database → R2 in one query. Datadog evaluated but rejected as priced for larger orgs. |
| T-066 | Feature flags + product analytics — PostHog single install. | not-started | 3 | 1–2 hr | — | 2026-04-27 audit | PostHog (open source, generous free tier) covers feature flags AND product analytics. Self-hosted EU instance evaluated; cloud EU region adequate for now. |
| T-067 | CDN cache strategy — explicit `Cache-Control` headers for venue assets. | not-started | 3 | 1 hr | T-001 | 2026-04-27 audit | Define `Cache-Control` on R2 object uploads for `.ply`, `.spz`, `manifest.json`, etc. Cache-busting via versioned paths (asset-version IDs already in the bundle URL structure). Document at `docs/operations/cdn-cache-strategy.md`. |
| T-068 | Disaster recovery runbook — one page covering every infra dependency. | not-started | 3 | 2 hr | T-062 | 2026-04-27 audit | One-page runbook at `docs/operations/disaster-recovery.md` covering Vercel outage, Neon regional failure, Railway outage, Cloudflare outage, GitHub account compromise, R2 unavailability. Each scenario: detection signal, recovery procedure, communication plan, expected RTO/RPO. Living document. |
| T-069 | Architecture notes — Railway region check + Clerk migration future option. | not-started | 2 | 0.5 hr | — | 2026-04-27 audit | Two small architectural notes. (a) Confirm Railway service region matches Neon `eu-west-2` — fix if mismatched. (b) Add a Clerk → Better-Auth migration ADR at `docs/architecture/adr/D-NNN.md` (next free number) noting trigger condition (Clerk monthly bill exceeds two days of an engineer's salary) and migration plan outline. |
| T-070 | Geass v3 — proactive surveillance layer. | deferred | 4 | 5–10 | T-054 | 2026-04-27 architectural review | Longitudinal awareness — surfaces things the founder didn't ask about. Weekly review prompts, anomaly detection against historical baseline, drift detection on test counts and customer signals, reminders on engineering rhythm (e.g. "no customer-facing feature in N weeks"). Reactivation trigger: T-054 status = `done` AND ≥ 14 days of event history in `sentinel.events` table. |
| T-071 | Geass ADR — agent architecture and trust boundary as D-017. | not-started | 3 | 0.5 | T-054 | 2026-04-27 architectural review | Document Geass agent architecture as ADR D-017 in `docs/architecture/adr/`. Covers event sources, judgment layer, delivery surfaces, trust boundary, naming rationale (the Code Geass reference and why the agent is framed as authority not observer), Discord-as-delivery decision rationale. Write concurrently with T-054 implementation start, not before. |
| T-072 | Email template system — editorial-quality transactional emails. | not-started | 4 | 2–3 | T-063 | 2026-04-27 audit (S+ tier philosophy applied) | Templates for welcome, password reset, partnership invitations (Trades Hall first), capture certification notifications, training-complete updates, billing receipts. Same editorial bar as the rest of the product (Cormorant Garamond + Inter, warm cream substrate, restrained gold accent, sentence case). React Email or similar, sent via Resend. NOT a wrapper around generic SaaS templates — bespoke, on-brand. Tracked separately from T-063 because the template work is substantial and benefits from sender domain being live first so each template can be live-tested. |
