...
| T-113 | Fix grouped table/chair movement parity between 2D and 3D planner views. | in-progress | 4 | 1 | — | USER-2026-04-29 | User report: 10-chair table group renders with chairs poking through a wall in 2D despite space in 3D, and moving the table in 2D moves the table/cloth but leaves grouped chairs at their original 3D positions. Ensure grouped items keep accurate transforms between 2D and 3D regardless of which viewer performs the move. |
...
| T-081 | Fix lint error in `grand-hall-textures.ts:342`. | not-started | 2 | 0.25 hr | — | AUDIT-CHATGPT-2026-04-27 (F-008) | `@typescript-eslint/restrict-template-expressions` failure. Convert the number explicitly. Trivial fix, immediate. |
