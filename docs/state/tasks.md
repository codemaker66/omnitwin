...
| T-087 | Three.js 0.170 → 0.180 upgrade + add Spark dependency. | not-started | 5 | 2–5 | — | AUDIT-CHATGPT-2026-04-27 (F-002), D-002, T-052 | Upgrade `three`, `@react-three/fiber`, `@react-three/drei` as one compatibility unit. Add Spark 2.0 dependency. Create a minimal splat fixture route at `/dev/splat-fixture`. Make it a CI smoke gate. **Supersedes T-052.** |
...
| T-098 | Dependency reproducibility pass. | not-started | 2 | 1 | T-080, T-087 | AUDIT-CHATGPT-2026-04-27 (F-019) | Align engine to Node 22 (currently README says ≥ 22, root says ≥ 20). Pin critical graphics, auth, build dependencies. `@types/three` must match runtime `three`. Add a dependency-update policy doc. |
