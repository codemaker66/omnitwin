...
**Standing delivery policy — 7 September 2026:** follow the
[build, ship and verify contract](../../.claude/conventions/shipping-changes.md).
Requested product changes include their verified release. Existing access,
credentials and release ownership are execution details to resolve; an OWNER
label is not a requirement for Blake to act when authorized tooling can perform
the step. Complete relevant backup, migration, build and live checks without
requesting an already-authorized deployment again.
...
The agent executes release steps using existing authorized tools and access. An **[OWNER]** label identifies an access or decision dependency; it does not require a human when that dependency is already satisfied. Ask only for a concrete missing input or authority.
...
- **2026-09-02:** production migration ledger is **`0063_phase_layout_snapshot_immutability`** (0062 and 0063 applied by this procedure at 00:2x BST from the D: ship worktree; state report before: 0061, after: 0063). The owner-only Neon backup branch was NOT taken first — recorded as a deviation. The 2026-08-18 `railway up` had never re-stamped `BUILD_GIT_SHA` (production reported the 25 July SHA 154c6894 while behaving like a later build), so §6's "gitSha matching is conclusive" was false until the next stamped deploy. The first stamped redeploy attempt (deployment 9722d24e, tree d77d7f3b) FAILED in the image build: the Dockerfile's `ARG PNPM_VERSION` was declared before `FROM` and therefore empty inside the stages, so the image had always installed the latest pnpm; pnpm 11 rejects a 9.15.4 lockfile. Fixed by re-declaring the ARG in both stages (see `.claude/gotchas/dockerfile-arg-scope.md`). Ship worktrees now live on D: (`git worktree add D:/claude/ship-<sha> <sha> --detach`); C: has ~11 GB free.
