...
- [ ] **Check master drift**: `git fetch origin && git merge-base --is-ancestor origin/master HEAD && echo ok`. Master keeps moving through Vercel's no-CI-wait path (5 commits within hours of the first reconciliation). T-525 re-reconciled up to `eb2f7431` (2026-07-17, merges eb127615 + 53541ad0, no conflicts, all gates green) — but re-check at deploy time; if master moved again, merge it into the branch FIRST and re-run the gates.
- [ ] **Coherence status (FULLY RESOLVED 2026-07-18, T-526):** five commits across two sessions (Diary lane `481bc2e3` + `6717e373`; Foundry `8cd027fa` + `790074c5` + `6353a9fe`) closed every committed-tree break. Verified from a TRUE fresh checkout (real `pnpm install --frozen-lockfile`, short-path worktree): web typecheck 0 / lint clean / units 3176 / build green; api build 0 errors / suite green. **Self-check before pushing** (do this, not a junction shortcut — junctions go stale when manifests change, and deep temp paths break vitest on Windows): `git worktree add C:/Users/blake/omnitwin2-fv HEAD --detach && cd C:/Users/blake/omnitwin2-fv && pnpm install --frozen-lockfile && pnpm --filter @omnitwin/api build && pnpm --filter @omnitwin/api test && pnpm --filter @omnitwin/web typecheck` — everything green, then `git worktree remove` it.
...
This is exactly what deploy.yml will do later — done deliberately first, so schema precedes code. It applies `0044→0048` (event architect / mission control — their code is in this deploy too), then `0049` (Reconstruction Foundry tables — journaled since T-526, so the standard path now handles it; the Slice-5 "hand-apply 0049" caveat is retired), then `0050→0051` (the Diary). All are additive; the Diary migrations were rehearsed end-to-end (Slice 4/5).
