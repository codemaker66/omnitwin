...
- [ ] **BLOCKER until the Foundry workstream commits (found 2026-07-17, T-525):** the committed api tree does not build from a fresh checkout — `routes/reconstruction-foundry.ts` + `services/reconstruction-foundry-integrations.ts` are committed but import the still-untracked `services/reconstruction-foundry.ts` and seven uncommitted `FOUNDRY_*` env fields. CI on any push fails at api build until they commit the module + env block together (their session has been messaged; details in `docs/sessions/2026-07-17.md`). **Self-check before pushing:** `git worktree add <tmp> HEAD --detach`, junction node_modules, `pnpm --filter @omnitwin/api build` — must be green.
