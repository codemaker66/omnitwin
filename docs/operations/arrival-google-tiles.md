...
**Stall watchdog (added by the robustness wave):** every tiles failure the library can *name* arrives as a `load-error` event and is already handled. A request that simply never answers is the exception — no `tiles-load-end` (nothing finished), no `load-error` (nothing failed) — and before this the phase machine sat in `loading` forever: no flight, no fallback, no diagnostic. `GoogleTilesStage`'s wiring effect now runs a **dead-man's switch**: it fires after `ARRIVAL_TILES_STALL_MS` (`arrival-config.ts`, currently 30 s) of **complete silence**, writes one plain-English `console.error`, and takes the same `fail("tiles")` fallback as everything else — the static hero photo carries the page. It is re-armed by every event that proves bytes are moving (`tiles-load-start`, `tile-download-start`, `load-tileset`, `load-model`), so it measures silence rather than total load time and **cannot** fire on a slow-but-working connection; it is disarmed permanently once the tiles report ready, so a slow patch mid-flight never takes the flight away. Operationally: if you see that line, the request is *pending*, not rejected — check the Network panel for an outstanding `tile.googleapis.com` request (hung connection, captive portal or proxy, extension or CSP block). A wrong, revoked or over-quota key does **not** look like this; it answers immediately and produces the key-specific diagnostic instead.
