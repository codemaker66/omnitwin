...
flowchart TD
    ...
    subgraph A3 [a3 · this week — viewer presentation polish + trust doctrine]
        ...
        T447(["T-447 - unified planner undo/redo"])
        T448(["T-448 - editor E2E suite repair"])
        T449(["T-449 - decouple visual-route E2E from API"])
        T464(["T-464 - runtime canvas demand rendering"])
        T465(["T-465 - measured route frame budget"])
        T466(["T-466 - mobile/tablet frame budget"])
        T467(["T-467 - Reception Room SPZ primary visual"])
    ...
    subgraph A7 [a7 · grand hall visual super-pipeline]
        T540(["T-540 - exact Grand Hall runtime + server-bound intake"])
        T541(["T-541 - RoomScene compositor + lineage slice"])
        T542(["T-542 - reviewed structural authority"])
        T543(["T-543 - professional captured-source pack"])
        T544(["T-544 - captured-master bake-off"])
        T545(["T-545 - Hero Volume pilot"])
        T546(["T-546 - materials + lighting"])
    ...
    subgraph A8 [a8 · grand hall evidence-to-v2 continuation]
        ...
        T565(["T-565 - panorama-to-E57 orientation proposals"])
    end
    ...
    subgraph G [g · ss++ platform execution programme]
        T428(["T-428 — CRM opportunity spine"])
        T435(["T-435 — supplier coordination foundation"])
        T453(["T-453 — first real room runtime package"])
        %% 2026-06-16: reviewed transform artifact registry path exists; signed Reception Room transform evidence still open.
        ...
        T463(["T-463 — revenue analytics v0"])
    end
    ...
    class T160,T480,T481,T482,T483,T484,T485,T541,T551,T552,T553,T559,T560,T561,T563,T564,T565 done
    class T550,T562 inprogress
...
or admission gate. Its real receipt-last build and independent full
recomputation check are complete; all 148 rows remain human-pending. T-561 is
the separately versioned all-source visual-scope successor. Its current
2048×1024 display-resolution agent observations record Grand Hall pixels in
sweeps
001–061, 065–075 and 148–149 (74 exact sources), and no Grand Hall pixels
observed in 062–064, 076–092 and 094–147 (74 exact sources); sweep 093 is
absent from the source inventory. Zero agent uncertainty flags do not establish
human review or native-pixel authority: `nativeResolutionHumanReviewCompleted`
remains false, every source remains human-pending, and T-561 cannot resolve the
two T-560 correspondence ambiguities. The preserved v1 50/98 T-554 review path
is stopped and superseded for acceptance; T-554 now also depends on a completed
T-561 successor, qualified human review, and exact 8192×4096 masks for every
human-included source. T-562 now preserves the clean browser SOG/SPZ/PLY bundle,
the successful exact-target native v14 capture, and their strict native-inclusive
comparison, while keeping that one-camera diagnostic separate from human scope
acceptance, difficult-oblique coverage, captured-master selection, and metric
...
training eligibility, reconstruction input, or runtime use. T-565 then fits
the eight exact external panoramas to those independently recovered native
faces with deterministic five-fold spherical validation and exposes the
sweep-047/scan-010 revisit beside the stronger scan-046 primary. It produces
only human-pending correspondence, station, orientation and candidate-pose
evidence; it does not accept a panorama pixel, room boundary, mask, transform,
training/reconstruction input, or runtime use. No push,
merge, deployment,
...
instead of running a continuous frame loop. It does not close broader public
runtime exposure or device FPS validation.

`T-465` records the measured 2026-06-17 route frame-budget pass. It keeps
Spark, Clerk, and public-route Three loads demand-routed based on the
Playwright/CDP profile of `/plan`, `/dev/trades-hall-visual`, and the public
room route; it still does not claim device-farm 60fps coverage.

`T-466` records the mobile/tablet repeat of the 2026-06-17 route frame-budget
pass. It adds narrow-viewport lean canvas budgets for planner and internal
visual routes, and records that the public Reception Room route was fallback
only because T-453 signed public runtime exposure evidence is still unavailable.
The real signed Reception Room runtime package needs a repeat pass once T-453
lands.

`T-467` records the 2026-06-18 Reception Room SPZ primary visual check: the
operator path points at `reception-room_xgrids_lcc2_spz_visual`, keeps
`spz_with_mesh` out of the primary visual layer, preserves the SOG export as
backup/provenance, and keeps T-091/T-091A and public exposure open.
...
`docs/strategy/ss-plus-plus-platform-execution-plan.md`, plus the completed
CRM (`T-428`) and revenue analytics (`T-463`) prerequisites that unblock later
onboarding and enterprise analytics work. It intentionally shows only
high-signal dependencies that are already visualised elsewhere; full
dependencies remain authoritative in `docs/state/tasks.md`.
