async (page) => {
  // Local read-only servers:
  //   5190 -> Quality LccStudio project /output
  //   5191 -> Mobile LccStudio project /output
  ...
  const variants = [
    ...
    {
      id: "mobile-sh0-sog-leaf",
      urls: [
        mobileRoot + "/render2/data/3dgs/0_14_0_2.sog",
        mobileRoot + "/render2/data/3dgs/0_3_0_0.sog",
        mobileRoot + "/render2/data/3dgs/0_8_0_0.sog",
        mobileRoot + "/render2/data/3dgs/0_9_0_0.sog",
      ],
    },
    {
  ...
  for (const variant of variants) {
    for (const view of views) {
      ...
      const path =
        "output/playwright/reception-hd-investigation/matrix-" +
        view.id +
        "-" +
        variant.id +
        ".png";
      await page.screenshot({ path, scale: "device", type: "png" });
      results.push({ variant: variant.id, view: view.id, path, fixture });
    }
  }
  return results;
}
