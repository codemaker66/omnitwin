...
import {
  ...
  type GrandHallT554FinalizedNativeSourceEpochV1,
...
const SessionRootSchema = z.string().min(1).refine(
  (value) =>
    isAbsolute(value) &&
    !value.includes("\0") &&
...
const ApplyMaskEditInputSchema = z
  .object({ ...MutationGuardShape, edit: GrandHallT554NativeReviewMaskEditV2Schema })
  .strict();
...
interface MaskWorkflowSessionSeamsV2 {
  ...
  readonly afterMaskChildStartPublishedBeforeRootVerification?: () =>
    | Promise<void>
    | void;
...
async function directKind(
  ...
  try {
    ...
    if (stats.isSymbolicLink()) {
      throw fail("CRASH_RECOVERY_REQUIRED", "A trusted workflow path is a symbolic link.");
    }
...
async function readSessionScope(
  ...
  try {
    ...
    if (errnoCode(error) === "ENOENT") {
      throw fail("ROOT_MISSING", "The native-review session root does not exist.", error);
    }
    throw error;
  }
...
function mapMaskStoreError(error: unknown): GrandHallT554NativeReviewMaskWorkflowSessionV2Error {
  ...
  if (error instanceof GrandHallT554NativeMaskStoreError) {
    if (error.code === "REVISION_CONFLICT") {
      return fail("MASK_REVISION_CONFLICT", "Mask revision CAS is stale.", error);
    }
    if (
      ...
      return fail("ARGUMENT_INVALID", "Mask edit or freeze request was rejected.", error);
    }
  }
  return fail("RESOURCE_FAILURE", "Native mask operation failed closed.", error);
}
...
function maskResumeSourceEpochBindings(input: {
  ...
  return {
    ...
    publicationReceipt:
      input.dependencies.registry.binding.publicationReceipt,
...
function assertFinalizedMaskResumeEpoch(
  finalized: GrandHallT554FinalizedNativeSourceEpochV1,
  snapshot: GrandHallT554NativeSourceEpochSnapshotV1,
  sourceEpochBindingSha256: Sha256,
): void {
  const observedSchemaVersion: string = finalized.schemaVersion;
  const observedDisposition: string = finalized.disposition;
  if (
    observedSchemaVersion !==
      "venviewer.grand-hall-t554-finalized-native-source-epoch.v1" ||
    observedDisposition !== "finalized_stable" ||
    finalized.sourceEpochNonceSha256 !== snapshot.sourceEpochNonceSha256 ||
    finalized.renderGeneration !== snapshot.renderGeneration ||
    finalized.epochBindingSha256 !== sourceEpochBindingSha256 ||
    !canonicalEqual(finalized.sourceVerification, snapshot.sourceVerification)
  ) {
    throw fail(
      "RESOURCE_FAILURE",
      "Mask-edit resume source finalization differs from its verified epoch.",
    );
  }
}
...
async function prepareMaskResumeCustody(input: {
  readonly active: ActiveCoordinatorSource;
  readonly sourceEpochNonce: string;
  readonly renderGeneration: number;
  readonly dependencies: MaskWorkflowSessionDependenciesV2;
}): Promise<GrandHallT554NativeReviewSourceCustodyBindingV2> {
  ...
  try {
    ...
    const finalized = await epoch.finalize();
    assertFinalizedMaskResumeEpoch(
      finalized,
      snapshot,
      sourceEpochBindingSha256,
    );
    ...
    throw fail(
      "RESOURCE_FAILURE",
      "Mask-edit resume source verification failed closed.",
      error,
    );
  }
}
...
function hasPartialPublicationAbortForMaskState(input: {
  ...
  for (const coordinatorEvent of input.events) {
    if (coordinatorEvent.eventType === "mask.freeze-intended.v2") {
      intents.set(
        coordinatorEvent.payload.operationIdSha256,
        coordinatorEvent,
      );
      continue;
    }
...
function rehydrateMaskStore(input: {
  ...
  try {
    ...
    if (!exactMaskStateMatches(workflow.event.payload.initialMaskState, exact)) {
      throw fail(
        "INTERNAL_INVARIANT_FAILED",
        "Durable mask workflow initial state differs from exact raster replay.",
      );
    }
    for (let index = workflow.index + 1; index < input.events.length; index += 1) {
      const candidate = input.events[index];
      if (
        candidate?.eventType !== "mask.edited.v2" ||
        !stableCustodyMatches(candidate.payload.sourceCustody, input.sourceCustody)
      ) {
        continue;
      }
...
function assertPreparedMatchesState(
  ...
  const parsed = GrandHallT554NativeReviewPreparedMaskBindingV2Schema.parse(
    prepared,
  );
  if (
...
class GrandHallT554NativeReviewMaskWorkflowSessionControllerV2
  implements GrandHallT554NativeReviewMaskWorkflowSessionV2
{
  ...
  #assertMutableLifecycle(): void {
    if (this.#coordinator().lifecycle === "stopped") {
      throw fail("SESSION_STOPPED", "Stopped native-review session is immutable.");
    }
    if (this.#coordinator().lifecycle === "poisoned") {
      throw fail("SESSION_POISONED", "Poisoned native-review session is immutable.");
    }
  }
  ...
  beginMaskWorkflow(
    ...
    return this.#lane.run(async () => {
      ...
      const sourceCustody = await prepareMaskResumeCustody({
        active,
        sourceEpochNonce: parseInput(
          NonceSchema,
      ...
      if (operationError !== undefined) {
        if (cleanupError !== undefined) {
          throw fail(
            "RESOURCE_CLEANUP_FAILED",
            "Initial mask-store cleanup failed after workflow-start error.",
            { operationError, cleanupError },
          );
        }
      ...
      if (cleanupError !== undefined) {
        throw fail(
          "RESOURCE_CLEANUP_FAILED",
          "Initial mask-store cleanup failed.",
          cleanupError,
        );
      }
      if (result === undefined) {
      ...
      try {
        ...
        const invalidatedMaskEvidence =
          active.phase === "mask_review"
            ...
            : null;
        if (active.phase === "mask_review" && invalidatedMaskEvidence === null) {
          throw fail(
            "INTERNAL_INVARIANT_FAILED",
        ...
        await this.#appendCoordinator(
          event<MaskEditedEvent>("mask.edited.v2", {
            ...
            invalidatedMaskJournal:
              invalidatedMaskEvidence?.checkpoint ?? null,
          }),
        );
  ...
  freezeMask(
    ...
    return this.#lane.run(async () => {
      ...
      try {
        ...
        await this.dependencies.seam
          ?.afterMaskChildStartPublishedBeforeRootVerification?.();
...
async function appendFreezeRecoveryAbort(input: {
  ...
  readonly publicationDisposition:
    ...
    | "reason_map_only"
    | "mask_and_reason_map";
  readonly abandonedMaskJournal:
    | GrandHallT554NativeReviewMaskChildCheckpointV2
    | null;
  readonly dependencies: MaskWorkflowSessionDependenciesV2;
}): Promise<void> {
  await assertGrandHallT554NativeReviewSessionOwnerV2({
  ...
  await input.coordinatorJournal.append({
    ...
    >>("mask.freeze-recovery-aborted.v2", {
      ...
      consumedRenderGeneration:
        input.intent.payload.allocatedRenderGeneration,
...
async function recoverPendingMaskFreeze(input: {
  ...
  try {
    ...
    if ((partial || publicationDisposition === "none") && hasAnyChildCustody) {
      throw fail(
        "CRASH_RECOVERY_REQUIRED",
    ...
    const frozenBindingSha256 =
      computeGrandHallT554NativeReviewFrozenMaskBindingV2Sha256(frozenBinding);
    const scope = maskScope({
      sessionScope: input.sessionScope,
      browserEpochNonceSha256: payload.browserEpochNonceSha256,
      coverageSegmentIdSha256: payload.coverageSegmentIdSha256,
...
async function openInjectedMaskWorkflowSession(
  ...
  try {
    ...
    if (store.coordinator.lifecycle === "stopped") {
      throw fail("SESSION_STOPPED", "Stopped native-review session cannot enter mask workflow.");
    }
    if (store.coordinator.lifecycle !== "active") {
      throw fail("SESSION_POISONED", "Poisoned native-review session cannot reopen.");
    }
    const active = store.coordinator.activeSource;
    if (
      active === null ||
    ...
    if (store.coordinator.activeSource?.phase === "mask_edit") {
      store = await resumeMaskEditEpochAfterBrowserRotation({
        sessionRoot,
        sessionScope,
        lease,
        coordinatorJournal,
