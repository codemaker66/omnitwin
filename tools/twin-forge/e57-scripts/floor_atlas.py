...
def _coarse_factor(grid: AtlasGrid, coarse_mm: float) -> int:
    ...
    own resolution — and a coarser cell averages away the sub-pixel sampling
    noise that a one-parameter model cannot explain anyway.
    """
    f = int(round(coarse_mm / grid.mm_per_px)) if grid.mm_per_px > 0 else 1
    return max(1, min(f, grid.height, grid.width))
...
def _block_sum(a: np.ndarray, f: int, ch: int, cw: int) -> np.ndarray:
    ...
    reference REGISTERED with the raster being fitted to it: a block's centroid
    is the coarse pixel centre, whereas a strided sample sits (f-1)/2 fine
    pixels off it in both axes — at 5 mm pixels and f=5 that is a fixed 10 mm
    diagonal shift, and a search that can only move texture radially pays for
    that shift with a bogus δ. Summing also keeps the cell a true WEIGHTED mean
    of every contribution inside it, which is exactly what the accumulators
    already hold.
    """
    a = a[: ch * f, : cw * f]
    if a.ndim == 2:
...
def _fit_pose_dz(
    ...
    """Direct search for the height correction that lands this sweep's planks
    on the reference's. Scored by normalised correlation of the high-pass —
    the quantity the atlas is actually judged on, not a proxy for it.

    Returns 0.0 when no candidate ever had enough independent overlap to be
    scored: floor nobody else saw must not be handed a confident correction.
    """
    ...
    for k in range(-n, n + 1):
        ...
        rgb, w = _sample_source(
            raster, Ck, cg, z_floor, self_blind_m, max_incidence_deg, None
        )
        m = (w > 0) & ref_ok
        if int(m.sum()) < 200:
            continue
        ...
        score = float((a * b).sum() / den) if den > 1e-9 else -np.inf
        if score > best_score:
...
def _align_pose_heights(
    ...
    span_m: float = 0.030,
    step_m: float = 0.003,
    coarse_mm: float = 24.0,
) -> list[float]:
    """Recover every sweep's height error against a LEAVE-ONE-OUT consensus.

    Why one parameter is the right model: if a sweep's centre sits δ above
    where its pose claims, every ray it casts lands long by δ·tan(incidence),
    ...
    The correction belongs to the POSE, not to the floor. The floor is one
    shared plane that every sweep must agree on; it is the sweeps that
    disagree, so it is the sweeps that move. Returned values are metres to add
    to each source's centre z.

    LEAVE-ONE-OUT is not a refinement here, it is the difference between a
    biased fit and an honest one. A source is 1/N of the consensus it is being
    scored against, and it agrees with ITSELF perfectly at δ=0 however wrong
    its pose is — so with a handful of sweeps the score carries a spurious peak
    at zero and the search is pulled into it. Removing the source's own
    weighted vote first leaves a reference that is independent evidence.

    Memory stays bounded, which is the constraint that lets this run over a
    whole building: per-source rasters are never stacked, only the ONE source
    being excluded is re-sampled, and it is re-sampled from a raster the search
    has to load anyway, so a lazily-loaded 8192 tier is not re-read.
    """
    from scipy import ndimage

    f = _coarse_factor(grid, coarse_mm)
    ...
    for img, C in sources:
        ...
        rgb, w = _sample_source(
            raster, C, grid, z_floor, self_blind_m, max_incidence_deg, None
        )
        ref_w = block_w - _block_sum(w, f, ch, cw)
        ref_acc = block_acc - _block_sum(rgb * w[..., None], f, ch, cw)
        # A cell only THIS source ever saw has no independent reference left.
        # Excluded from the correlation rather than scored as zero: an empty
        # reference is missing evidence, and counting it as agreement would
        # smuggle back in the self-vote this pass exists to remove.
        ref_ok = ref_w > 1e-9
        ref = (ref_acc / np.where(ref_ok, ref_w, 1.0)[..., None]).mean(axis=2)
        # Flat-fill the excluded cells before the high-pass. Left at zero they
        # are a cliff edge, and the filter spreads that artificial edge back
        # over cells that ARE valid — manufacturing structure for the search to
        # lock onto right where the evidence ran out.
        ...
        dzs.append(
            _fit_pose_dz(
                ...
                self_blind_m, max_incidence_deg, span_m, step_m,
            )
        )
    return dzs
...
def accumulate_floor_atlas(
    ...
    Between the passes, `align` recovers the per-sweep pose-height error that
    otherwise smears the fusion: a wrong centre height throws a grazing look
    centimetres sideways, so many views land the same plank in many places and
    the mean ERASES the texture it was supposed to sharpen.

    Returns (atlas float32 HxWx3, report). The report carries `observed`
    (bool), `counts` (int per pixel), `align_dz` (metres to add to each
    source's centre z), `covered_frac` and `rejected_frac` — unobserved floor
    ...
    # --- alignment pass: recover each sweep's pose-height error ------------
    dzs = (
        _align_pose_heights(
            sources, grid, z_floor, acc, wsum,
    ...
    for si, (img, C) in enumerate(sources):
        Ci = np.asarray(C, dtype=np.float64)
        if dzs[si]:
            Ci = Ci.copy()
            Ci[2] += dzs[si]
        rgb, w = _sample_source(
            img, Ci, grid, z_floor, self_blind_m, max_incidence_deg,
            occluder,
        )
        seen = w > 0
        total += int(seen.sum())
        if harmonise:
            rgb = _harmonise_to_consensus(
                rgb, seen, mean_rgb, grid, Ci, z_floor
            )
    ...
    report = {
        "observed": observed,
        "counts": counts,
        # metres to add to each source's centre z — a POSE correction, usable
        # by anything else that consumes these sweeps, not an atlas-local knob
