import { afterEach, describe, expect, it } from "vitest";
import { cleanup, fireEvent, render, screen } from "@testing-library/react";
import { useCockpitStore } from "../../../../stores/cockpit-store.js";
import type { PlannerLayerPolicy } from "../../../../lib/planner-layer-composition.js";
import { CockpitNavRail } from "../CockpitNavRail.js";

const CONFIGURABLE_POLICY: PlannerLayerPolicy = {
  kind: "configurable",
  effectiveMode: "hybrid",
  controlsLocked: false,
};
const CAPTURED_ONLY_POLICY: PlannerLayerPolicy = {
  kind: "captured-only",
  effectiveMode: "splat",
  controlsLocked: true,
};

afterEach(() => { cleanup(); useCockpitStore.getState().reset(); });

describe("CockpitNavRail", () => {
  it("renders eleven lens buttons with Design pressed by default", () => {
    render(<CockpitNavRail layerPolicy={CONFIGURABLE_POLICY} />);
    const buttons = screen.getAllByRole("button", { name: /design|guests|flow|evidence|lighting|power|rigging|av|ops|costs|share/i });
    expect(buttons.length).toBeGreaterThanOrEqual(11);
    expect(screen.getByRole("button", { name: /design/i }).getAttribute("aria-pressed")).toBe("true");
  });

  it("switches the active lens in the store on click", () => {
    render(<CockpitNavRail layerPolicy={CONFIGURABLE_POLICY} />);
    fireEvent.click(screen.getByRole("button", { name: /flow/i }));
    expect(useCockpitStore.getState().activeMode).toBe("flow");
    expect(screen.getByRole("button", { name: /flow/i }).getAttribute("aria-pressed")).toBe("true");
  });

  it("shows source status instead of operational lens buttons when geometry is unavailable", () => {
    render(<CockpitNavRail layerPolicy={CAPTURED_ONLY_POLICY} />);
    expect(screen.getByTestId("captured-source-rail-status").textContent).toContain("Source");
    expect(screen.queryByRole("button", { name: /flow|guests|evidence|ops|costs|share/i })).toBeNull();
  });

  it("does not label an identity-pending room as captured source", () => {
    render(<CockpitNavRail layerPolicy={{
      kind: "identity-pending",
      effectiveMode: "hybrid",
      controlsLocked: true,
    }} />);
    expect(screen.getByTestId("captured-source-rail-status").textContent).toContain("Resolving");
    expect(screen.queryByText("Source")).toBeNull();
  });
});
