...
// Only annotation/constraint work is bounded by viewport or camera motion.
// Model geometry, materials and applied table dressing retain identical fidelity.
export const COMPACT_PLANNER_OVERLAY_MIN_VIEWPORT_WIDTH = 1100;
/**
...
export function shouldLimitPlannerFurnitureOverlays(
  viewportWidth: number,
  cameraInteractionActive = false,
): boolean {
  return cameraInteractionActive || viewportWidth < COMPACT_PLANNER_OVERLAY_MIN_VIEWPORT_WIDTH;
}
...
/** Keep the inspected hierarchy out of the detailed instanced batch. */
export function plannerFurnitureRenderPartition(
  ...
  return { instancedItems, instancedIds };
}
...
export function shouldRenderIndividualFurnitureModel(options: {
  ...
  return options.inspected || !options.instanced || options.instancingFailed;
}
...
const PlacedFurnitureItem = memo(function PlacedFurnitureItem({
  ...
  return (
    <group name={`furniture-${placed.id}`}>
      ...
      {appliedClothStyle !== null && isAnimating && (
        <group
          position={[placed.x, placed.y, placed.z]}
          rotation={[0, placed.rotationY, 0]}
          scale={presentationScale}
...
export function PlacedFurniture(): React.ReactElement {
  ...
  const limitFurnitureOverlays = shouldLimitPlannerFurnitureOverlays(size.width, cameraInteractionActive);
  ...
  const fullConstraintViolationIds = useMemo(
    ...
    [placedItems, roomDims, limitFurnitureOverlays],
  );
  const leanConstraintViolationIds = useMemo(
    () => (limitFurnitureOverlays && MAX_LEAN_CONSTRAINT_VIOLATION_SKINS > 0
      ...
      : EMPTY_VIOLATION_IDS),
    [placedItems, roomDims, selectedIds, limitFurnitureOverlays],
  );
  const constraintViolationIds = limitFurnitureOverlays
    ...
    <group name="placed-furniture">
      ...
      {placedItems.map((placed) => (
        <PlacedFurnitureItem
          ...
          renderNamePlate={canRenderNameplate(placed.id)}
