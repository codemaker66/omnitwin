// Frozen oracle: the per-mesh Grand Hall ornament JSX as it stood before the
// ornaments were described as data and merged into opaque stand-in batches
// (source: packages/web/src/components/GrandHallOrnaments.tsx at d7eb71b).
// GrandHallOrnaments.merge.test.tsx proves the new description renders the
// same per-mesh tree and the same triangles per material and surface. When the
// ornaments are intentionally redesigned, delete this file and its legacy
// comparisons; the stand-in versus per-mesh comparisons keep guarding merging.
...
export interface LegacyGrandHallOrnamentsProps {
  readonly width?: number;
  readonly length?: number;
  readonly height?: number;
}
 ...
 */
export function LegacyGrandHallOrnaments({
...
}: LegacyGrandHallOrnamentsProps): React.ReactElement {
