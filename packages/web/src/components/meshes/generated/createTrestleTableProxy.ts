...
function addTabletopDetails(
  ...
  const surface = addSurfaceDetail(
    ...
    new RoundedBoxGeometry(
      1.806,
      TABLETOP_VENEER_THICKNESS_METRES,
      0.736,
      1,
      0.001,
    ),
    ...
    [
      0,
      TABLETOP_THICKNESS_METRES / 2 - TABLETOP_VENEER_THICKNESS_METRES / 2,
      0,
    ],
    boxCollider([1.806, TABLETOP_VENEER_THICKNESS_METRES, 0.736]),
    options,
  );
...
function createTabletop(
  ...
  if (coreMesh === undefined) throw new Error("trestle tabletop is missing its core mesh");
...
function createTrestleSide(
  ...
  const feet = addFootCaps(side, rails.footBar, registry, materials, options);
  const fasteners = addHingeHardware(side, rails.topRail, registry, materials, options);
  return {
...
export function createTrestleTableProxy(
  ...
  registry.destructionGroups.stretcher = [stretcher.stretcher];
