...
describe("createRoundTableProxy", () => {
  ...
  it("creates individually named meshes for the top, frame, braces, hardware, and caps", () => {
    ...
    expect(names).toContain("front-left-leg-post");
    expect(names).toContain("right-rear-brace-strap");
    expect(names).toContain("hinge-fastener-a-front-lower-detail");
    expect(names).toContain("rear-right-leg-foot-cap-detail");
  });
  ...
  it("publishes the semantic hierarchy and destruction sets expected by the presentation runtime", () => {
    ...
    expect(table.userData.componentId).toBe("root");
    expect(runtime.nodes.tabletop?.name).toBe("tabletop__pivot");
    expect(runtime.nodes["front-left-leg"]?.parent).toBe(runtime.nodes["trestle-a"]);
    expect(runtime.nodes["right-rear-brace"]?.parent).toBe(runtime.nodes["trestle-b"]);
    expect(runtime.meshes.tabletop).not.toBe(runtime.meshes["trestle-a"]);
    expect(runtime.sockets["tabletop-centre"]?.parent).toBe(runtime.nodes.tabletop);
    expect(runtime.destructionGroups["frame-a"]).toContain(runtime.nodes["front-left-leg"]);
    expect(runtime.destructionGroups["frame-b"]).toContain(runtime.nodes["right-rear-brace"]);
    expect(runtime.destructionGroups["brace-set-a"]).toHaveLength(2);
    expect(runtime.destructionGroups["brace-set-b"]).toHaveLength(2);
    expect(runtime.destructionGroups["hinge-plate-a"]).toHaveLength(2);
    expect(runtime.destructionGroups["hinge-plate-b"]).toHaveLength(2);
    expect(runtime.destructionGroups.hardware).toContain(runtime.nodes["frame-crossbar"]);
    expect(runtime.destructionGroups.root).toEqual([
      runtime.nodes.tabletop,
      runtime.nodes["trestle-a"],
      runtime.nodes["trestle-b"],
      runtime.nodes["frame-crossbar"],
    ]);

    const presentation = createFurniturePresentationRuntime(table, { explodeDistance: 0.25 });
    const frontLeg = presentation.inspectionParts.find((part) => part.id === "front-left-leg");
    expect(frontLeg?.parentPartId).toBe("trestle-a");
  ...
  it("marks subordinate surface details to remain attached during exploded inspection", () => {
    ...
    expect(details.some((mesh) => mesh.name.includes("hinge-plate-"))).toBe(true);
  ...
  it("publishes the restored generated-reference provenance without measured authority", () => {
    ...
    expect(table.userData).toMatchObject({
      canonicalDimensionsMetres: [1.83, 0.76, 1.83],
      evidenceSource:
