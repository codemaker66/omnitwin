...
function addSurfaceDetail(
  ...
  requireRegisteredMesh(records, id).userData.explodeWithParent = EXPLODE_WITH_PARENT;
  return detail;
}
...
function createSocket(root: Group, name: string, position: Vector3Tuple): Object3D {
...
function createSockets(root: Group): Readonly<Record<string, Object3D>> {
  return {
    "floor-contact": createSocket(root, "floor-contact__socket", [0, 0, 0]),
    "seat-contact": createSocket(root, "seat-contact__socket", [0, 0.435, 0]),
    "backrest-mount": createSocket(root, "backrest-mount__socket", [0, 0.72, -0.209]),
  };
}
...
export function createBanquetChairProxy(
  ...
  root.userData.provenance = "img2threejs-banquet-chair-evidence";
  ...
  const sockets = createSockets(root);
