...
const PLATFORM_DEPTH_METRES = 1.02;
const PLATFORM_HEIGHT_METRES = 0.4;

const DECK_CORE_BOTTOM_Y = 0.36;
...
const CARPET_BORDER = 0.025;
const EDGE_TRIM_DEPTH = 0.03;
...
const FRAME_SHORT_RAIL_LENGTH = 0.95;
const FRAME_RAIL_THICKNESS = 0.044;
const FRAME_RAIL_CENTRE_Y = 0.338;
const FRAME_LONG_RAIL_Z = 0.475;
const FRAME_SHORT_RAIL_X = 1.1;
const CROSSMEMBER_LENGTH = 0.95;
...
const LOCK_BOLT_EMBED_DEPTH = 0.002;
...
const BRACE_INSET_Z = 0.26;
const BRACE_Z_OFFSET = 0.43;
...
export interface PlatformNarrowProxyOptions {
  readonly castShadow?: boolean;
  readonly receiveShadow?: boolean;
}

type PlatformProxyOptions = PlatformNarrowProxyOptions;
...
function createCarpetTexture(): DataTexture {
  ...
  texture.name = "platform-narrow-charcoal-carpet-weave";
  texture.colorSpace = SRGBColorSpace;
  texture.wrapS = RepeatWrapping;
  texture.wrapT = RepeatWrapping;
  texture.repeat.set(8, 3.4);
...
function createMaterials(): PlatformMaterials {
  ...
  carpet.name = "platform-narrow-charcoal-carpet";
  const deckCore = new MeshStandardMaterial({
    color: 0x151719, metalness: 0, roughness: 0.82, side: FrontSide,
  });
  deckCore.name = "platform-narrow-deck-core";
  const trim = new MeshStandardMaterial({
    color: 0x111315, metalness: 0.18, roughness: 0.58, side: FrontSide,
  });
  trim.name = "platform-narrow-black-edge-trim";
  const aluminium = new MeshStandardMaterial({
    color: 0xd7dadd, metalness: 0.5, roughness: 0.3, side: FrontSide,
  });
  aluminium.name = "platform-narrow-satin-aluminium";
  const lock = new MeshStandardMaterial({
    color: 0x4b5053, metalness: 0.72, roughness: 0.38, side: FrontSide,
  });
  lock.name = "platform-narrow-lock-metal";
  const fastener = new MeshStandardMaterial({
    color: 0xc4c8ca, metalness: 0.92, roughness: 0.25, side: FrontSide,
  });
  fastener.name = "platform-narrow-fastener-steel";
  const rubber = new MeshStandardMaterial({
    color: 0x151617, metalness: 0, roughness: 0.86, side: FrontSide,
  });
  rubber.name = "platform-narrow-rubber-foot";
  return { carpet, deckCore, trim, aluminium, lock, fastener, rubber };
}
...
function createPerimeterFrame(
  ...
  return [
    ...
    createBoxPart(root, registry, "right-rail", [
      FRAME_RAIL_THICKNESS, FRAME_RAIL_THICKNESS, FRAME_SHORT_RAIL_LENGTH,
    ], [FRAME_SHORT_RAIL_X, FRAME_RAIL_CENTRE_Y, 0], material, options),
  ];
}
...
function addLegHardware(
  ...
  const bolts = ([-1, 1] as const).map((xDirection) => {
    const bolt = createPart(
      ...
      materials.fastener,
      [
        xDirection * LOCK_BOLT_X_OFFSET,
        LOCK_PLATE_CENTRE_Y - LEG_POST_CENTRE_Y,
        zDirection * (
          LOCK_PLATE_FACE_Z
          + LOCK_PLATE_DEPTH / 2
          + LOCK_BOLT_DEPTH / 2
          - LOCK_BOLT_EMBED_DEPTH
        ),
      ],
      options,
...
function createLegs(
  ...
  return [
    ...
    createLeg(root, registry, "front-centre", 0, LEG_Z_OFFSET, materials, options),
    ...
    createLeg(root, registry, "rear-centre", 0, -LEG_Z_OFFSET, materials, options),
...
function createDiagonalBrace(
  ...
  if (mesh === undefined) throw new Error(`platform-narrow brace ${id} is missing its mesh`);
...
function createDiagonalBraces(
  ...
  for (const zDirection of [-1, 1] as const) {
    ...
    for (const xDirection of [-1, 1] as const) {
      const hand = xDirection < 0 ? "left" : "right";
      braces.push(createDiagonalBrace(
        root, registry, `${side}-centre-${hand}-diagonal-brace`,
        [0, BRACE_LOWER_Y, z],
        [xDirection * BRACE_INSET_X, BRACE_UPPER_Y, z],
        materials, options,
      ));
    }
  }
  for (const xDirection of [-1, 1] as const) {
    ...
    const hand = xDirection < 0 ? "left" : "right";
    for (const zDirection of [-1, 1] as const) {
      const z = zDirection * LEG_Z_OFFSET;
      const side = zDirection > 0 ? "front" : "rear";
      braces.push(createDiagonalBrace(
        root, registry, `${hand}-${side}-end-diagonal-brace`,
        [x, BRACE_LOWER_Y, z],
        [x, BRACE_UPPER_Y, z - zDirection * BRACE_INSET_Z],
        materials, options,
      ));
    }
  }
...
function publishDestructionGroups(
  ...
  registry.destructionGroups["leg-system"] = legs.map(({ leg }) => leg);
  registry.destructionGroups["brace-system"] = [...braces];
  registry.destructionGroups["corner-lock-system"] = legs.flatMap(({ hardware }) =>
    hardware.filter((part) => part.userData.componentId !== undefined
      && !String(part.userData.componentId).endsWith("-rubber-foot-detail")));
  registry.destructionGroups["foot-system"] = legs.flatMap(({ hardware }) =>
    hardware.filter((part) => String(part.userData.componentId).endsWith("-rubber-foot-detail")));
}

/**
 * Builds the evidence-derived, stylized narrow modular stage-platform proxy in metres.
 * The dedicated 1.02 metre depth is authored directly across the deck, frame,
 * leg stations, and braces. Hidden folding joints use a symmetric structural
 * approximation and remain presentation-only evidence.
 */
export function createPlatformNarrowProxy(
  options: PlatformNarrowProxyOptions = {},
): Group {
  const root = namedGroup("root");
  root.name = "platform-narrow-proxy";
  ...
  root.userData.approximationNotes =
    "Single-view generated reference; rear linkage, folding mechanics, and load rating are not measured.";
  ...
  addSocket(root, registry, "centre-underframe", [0, CROSSMEMBER_CENTRE_Y, 0]);
  addSocket(root, registry, "left-frame-coupling", [
    -PLATFORM_WIDTH_METRES / 2, EDGE_TRIM_CENTRE_Y, 0,
  ]);
  addSocket(root, registry, "right-frame-coupling", [
    PLATFORM_WIDTH_METRES / 2, EDGE_TRIM_CENTRE_Y, 0,
  ]);
  ...
  root.userData.referenceImageSha256 =
