...
describe("frozen layout room adapter", () => {
  it("centres the canonical lower-left room and applies the same offset to objects", () => {
    ...
    expect(model.renderDimensions).toEqual({ width: 42, length: 21, height: 7 });
    expect(model.furnitureOffset).toEqual([-21, 0, -10.5]);
    expect(fixtureObject).toBeDefined();
    expect((fixtureObject?.position.x ?? 0) * 2 + model.furnitureOffset[0])
      .toBe((fixtureObject?.position.x ?? 0) * 2 - 21);
    expect((fixtureObject?.position.z ?? 0) * 2 + model.furnitureOffset[2])
      .toBe((fixtureObject?.position.z ?? 0) * 2 - 10.5);
  });
