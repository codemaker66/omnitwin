...
describe("layout timeline model", () => {
  ...
  it("converts the same immutable canonical payload only once by object identity", () => {
    const first = placedItemsFromCanonicalSnapshot(CANONICAL_LAYOUT_SNAPSHOT_V0_FIXTURE);
    const repeated = placedItemsFromCanonicalSnapshot(CANONICAL_LAYOUT_SNAPSHOT_V0_FIXTURE);
  ...
  it("pairs a stable object ID before SKU and switches its render metadata at midpoint", () => {
    const from = [placed("stable", "chair", 0, 0)];
    const to = [placed("stable", "table", 10, 0)];
    const plan = buildTimelineItemTransitionPlan(from, to);

    expect(plan.pairs).toHaveLength(1);
    expect(interpolateTimelineItemTransitionPlan(plan, 0.49)[0]?.catalogueItemId).toBe("chair");
    expect(interpolateTimelineItemTransitionPlan(plan, 0.5)[0]?.catalogueItemId).toBe("table");
    expect(interpolateTimelineItemTransitionPlan(plan, 0.5)[0]?.x).toBe(5);
  });

  it("breaks equal-distance nearest-SKU ties by deterministic target ID", () => {
    const from = [placed("from", "chair", 0, 0)];
    const to = [placed("z-target", "chair", 1, 0), placed("a-target", "chair", -1, 0)];

    expect(buildTimelineItemTransitionPlan(from, to).pairs[0]?.to.id).toBe("a-target");
  });
