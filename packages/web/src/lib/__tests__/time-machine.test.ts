...
// Concretely typed rather than `ReplayObject` (which permits `unknown`
// values, so TypeScript cannot prove a fixture is JSON-serializable once it
// is embedded in an Action payload). Still assignable to ReplayObject.
// A `type` alias, not an `interface`: interfaces get no implicit index
// signature, so they satisfy neither JsonValue nor ReplayObject even when
// every member is JSON-safe. This is the fix, not a cast.
type PlanObject = {
  readonly id: string;
...
const TABLE: PlanObject = { id: "obj-table", kind: "table-round", positionX: 1 };
const CHAIR: PlanObject = { id: "obj-chair", kind: "chair", positionX: 4 };
