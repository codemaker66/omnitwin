...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
  <tr><td style="background:${BRAND_NAVY};padding:20px 24px;">
    ...
    <span style="color:rgba(255,255,255,0.5);font-size:13px;margin-left:12px;">${escapeHtml(title)}</span>
  </td></tr>
...
function row(label: string, value: string | null | undefined): string {
  ...
  return `<tr><td style="padding:4px 0;color:#999;font-size:13px;width:120px;vertical-align:top;">${escapeHtml(label)}</td><td style="padding:4px 0;font-size:13px;color:#333;">${escapeHtml(value)}</td></tr>`;
}
...
export function enquiryApproved(data: EnquiryApprovedData): { subject: string; html: string } {
  ...
  const bodyHtml = `
    ...
    <p style="font-size:14px;color:#333;line-height:1.5;">
      Great news! Your enquiry for <strong>${escapeHtml(data.spaceName)}</strong> at <strong>${escapeHtml(data.venueName)}</strong>
      ${data.eventDate !== null ? ` on <strong>${escapeHtml(data.eventDate)}</strong>` : ""} has been approved.
    </p>
    <p style="font-size:14px;color:#333;line-height:1.5;">
...
export function enquiryRejected(data: EnquiryRejectedData): { subject: string; html: string } {
  ...
  const bodyHtml = `
    ...
    <p style="font-size:14px;color:#333;line-height:1.5;">
      Thank you for your interest in <strong>${escapeHtml(data.spaceName)}</strong> at <strong>${escapeHtml(data.venueName)}</strong>${data.eventDate !== null ? ` on <strong>${escapeHtml(data.eventDate)}</strong>` : ""}.
      Unfortunately, we're unable to accommodate this particular request at this time.
    </p>
    ${data.note !== null && data.note !== "" ? `<div style="margin:16px 0;padding:12px;background:#fef2f2;border-radius:6px;border-left:3px solid #ef4444;font-size:13px;color:#555;"><strong>Note from the events team:</strong><br>${escapeHtml(data.note)}</div>` : ""}
    <p style="font-size:14px;color:#333;line-height:1.5;">
