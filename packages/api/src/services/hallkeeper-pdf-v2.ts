...
export async function generateSheetPdfV2(data: HallkeeperSheetV2): Promise<Buffer> {
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    const titleBottom = MARGIN + 10 + doc.heightOfString(data.config.name, { width: CONTENT_W - 80 });
    ...
    const gridY = Math.max(MARGIN + 48, titleBottom + 12);
    const col1X = MARGIN;
    ...
    if (data.timing !== null) {
      ...
      if (hasDeadlines) {
        ...
        for (const d of sortedDeadlines) {
          const label = `${PHASE_METADATA[d.phase].label} · ${fmtTime(d.deadline, data.venue.timezone)}`;
      ...
      // No live Diary booking holds this room, so there is no honest hour to
      // print. Say that on the page rather than leaving a silent gap the
      // hallkeeper could read as "no setup deadline".
      const boxY = headerBottom + 4;
      doc.save();
      doc.roundedRect(MARGIN, boxY, CONTENT_W, 28, 4).fillAndStroke("#faf8f2", GOLD_LIGHT);
      doc.restore();
      doc.font("Helvetica-Bold").fontSize(10).fillColor(GOLD);
      doc.text("Times not set", MARGIN + 12, boxY + 8);
      doc.font("Helvetica").fontSize(9).fillColor(INK_DIM);
      doc.text(
        "No booking in the Diary holds this room yet — confirm the time before setting up.",
        MARGIN + 160, boxY + 9,
      );
      ...
      for (const zoneGroup of phase.zones) {
        ...
        for (const row of zoneGroup.rows) {
          ...
          if (row.notes.length > 0) {
            ...
            doc.text(`Note: ${row.notes}`, textX, rowY + 16, { width: nameW });
          }
    ...
    for (let i = pages.start; i < pages.start + pages.count; i++) {
      ...
      if (i === 0 && qrBuffer !== null) {
        ...
        doc.text("Web view", MARGIN, footerTop + 43, { width: 36, height: 8, align: "center" });
      }

      // Venue + generation time (all pages)
      const textLeft = i === 0 && qrBuffer !== null ? MARGIN + 50 : MARGIN;
      doc.font("Helvetica").fontSize(6.5).fillColor(INK_FAINT);
      // Explicit bounded boxes keep PDFKit out of automatic body pagination.
      // The footer pass must never create pages after totalPages is captured.
      const footerTextWidth = A4_W - MARGIN - 90 - textLeft;
      doc.text(`${data.venue.name}  ·  ${data.venue.address}`, textLeft, footerTop + 10, { width: footerTextWidth, height: 10, ellipsis: true });

      // Venue-pinned like every other clock face on the sheet, so a printed
      // copy can be compared against the Diary without a zone conversion.
      const genTime = new Date(data.generatedAt).toLocaleString("en-GB", {
        day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit",
        timeZone: data.venue.timezone,
      });
      doc.text(`Generated by VenViewer  ·  ${genTime}  ·  ${data.venue.timezone}`, textLeft, footerTop + 22, { width: footerTextWidth, height: 10, ellipsis: true });
      ...
      if (data.approval !== null) {
        ...
        const versionLabel = `Approved v${String(a.version)}`;
        const versionWidth = Math.ceil(doc.widthOfString(versionLabel)) + 1;
        const dateWidth = Math.ceil(doc.widthOfString(approvedDate)) + 1;
        const nameX = textLeft + versionWidth + 8;
        const dateX = textLeft + footerTextWidth - dateWidth;
        // The audit identity may shorten; its version and timestamp must survive.
        doc.text(versionLabel, textLeft, footerTop + 34, { width: versionWidth, height: 10 });
        doc.text(a.approverName, nameX, footerTop + 34, { width: dateX - nameX - 8, height: 10, ellipsis: true });
        doc.text(approvedDate, dateX, footerTop + 34, { width: dateWidth, height: 10, align: "right" });
      }

      // Page number (all pages, right-aligned)
      doc.font("Helvetica-Bold").fontSize(8).fillColor(INK_DIM);
      doc.text(`Page ${String(i + 1)} of ${String(totalPages)}`, A4_W - MARGIN - 80, footerTop + 14, { width: 80, height: 12, align: "right" });
...
/**
 * Every clock face in the PDF is the VENUE's wall clock (Ship Friday gate
 * line 20). The renderer runs on a server whose own zone is an accident of
 * deployment, so an un-pinned `toLocaleTimeString` printed UTC on the API
 * host and Europe/London on a developer laptop — the same sheet, two
 * different "setup by" times. `timeZone` is the per-venue IANA identifier
 * carried on `data.venue.timezone` (venues.timezone, migration 0015).
 */
...
function renderInstructions(doc: Doc, ins: EventInstructions, timeZone: string): void {
  ...
  if (doorSummary !== null) {
    renderDoorSchedulePdf(doc, doorSummary, timeZone);
    anyRendered = true;
  }
...
function renderDoorSchedulePdf(doc: Doc, summary: DoorScheduleSummary, timeZone: string): void {
  // Door times were the last machine-local clock in the PDF: `getHours()`
  // reads the SERVER's zone, so an opening printed an hour early whenever the
  // host ran on UTC. Pin it to the venue like every other time on the sheet.
  const fmtDoorTime = (iso: string): string => {
    try {
  ...
  for (const entry of summary.entries) {
    ...
    if (entry.events.length === 0) {
      ...
      for (const ev of entry.events) {
        ...
        // time
        doc.font("Helvetica-Bold").fontSize(8.5).fillColor(INK);
        doc.text(fmtDoorTime(ev.at), MARGIN + 22, cursorY, { continued: true });
...
function rowHeightFor(doc: Doc, row: ManifestRowV2): number {
  ...
  const notesH = doc.heightOfString(`Note: ${row.notes}`, { width: nameW });
...
function renderApprovalBanner(
  ...
  const dateWidth = Math.ceil(doc.widthOfString(approvedDate)) + 1;
  doc.text(approval.approverName, MARGIN + leftW + 8, bandY + 5, {
    width: rightW - dateWidth - 8, height: 11, align: "right", ellipsis: true,
  });
  doc.text(approvedDate, A4_W - MARGIN - dateWidth, bandY + 5, {
    width: dateWidth, height: 11, align: "right",
  });
}
