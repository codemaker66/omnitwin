...
import type {
  ...
  HistoricalRuntimeExecutionKeyPolicy,
  HistoricalRuntimeExecutionObjectReceipt,
  HistoricalRuntimeExecutionStatement,
  ReconstructionDsseEnvelope,
  PhaseLayoutRuntimeBinding,
...
export const assetVersions = pgTable("asset_versions", {
  ...
  unique("asset_versions_exact_runtime_member_unique").on(
    table.id,
    table.venueSlug,
    table.roomSlug,
    table.fileName,
  ...
  unique("asset_versions_runtime_byte_identity_unique").on(
    table.id,
    table.venueSlug,
    table.roomSlug,
    table.sha256,
...
export const runtimePackages = pgTable("runtime_packages", {
  ...
  unique("runtime_packages_id_scope_digest_unique").on(
    table.id,
    table.venueSlug,
    table.roomSlug,
    table.contentDigest,
...
export const runtimeTransformArtifacts = pgTable("runtime_transform_artifacts", {
  ...
  artifactDigest: varchar("artifact_digest", { length: 64 }),
  reviewNote: text("review_note"),
  ...
  unique("runtime_transform_artifacts_exact_binding_unique").on(
    table.id,
    table.runtimePackageId,
    table.venueSlug,
    table.roomSlug,
    table.transformArtifactId,
...
export const runtimeQaRecords = pgTable("runtime_qa_records", {
  ...
  unique("runtime_qa_records_exact_binding_unique").on(
    table.id,
    table.runtimePackageId,
    table.venueSlug,
    table.roomSlug,
...
// Exact registered Scene Authority artifacts are defined here so the
// presentation admission can carry a real composite foreign key to them.
export const reconstructionReviewEvidenceArtifacts = pgTable("reconstruction_review_evidence_artifacts", {
  ...
  unique("reconstruction_review_evidence_exact_runtime_unique").on(
    table.id,
    table.venueSlug,
    table.artifactKind,
    table.artifactId,
...
// Normalized, append-only reviewed-presentation admission. Migration 0063
// makes these rows immutable and supplies the exact composite foreign keys.
// No production seed is provided: current runtime packages remain ineligible
// until their rights and Scene Authority causality is actually reviewed.
export const runtimePresentationRightsEvidence = pgTable("runtime_presentation_rights_evidence", {
  id: uuid("id").primaryKey().defaultRandom(),
  assetVersionId: uuid("asset_version_id").notNull(),
  venueSlug: varchar("venue_slug", { length: 100 }).notNull(),
  roomSlug: varchar("room_slug", { length: 100 }).notNull(),
  assetSha256: varchar("asset_sha256", { length: 64 }).notNull(),
  assetSizeBytes: bigint("asset_size_bytes", { mode: "number" }).notNull(),
  evidenceDigest: varchar("evidence_digest", { length: 64 }).notNull(),
  evidenceBody: jsonb("evidence_body").$type<Record<string, unknown>>().notNull(),
  ...
  foreignKey({
    columns: [
      ...
      table.roomSlug,
      table.assetSha256,
      table.assetSizeBytes,
    ],
    foreignColumns: [
      assetVersions.id,
      assetVersions.venueSlug,
      assetVersions.roomSlug,
      assetVersions.sha256,
      assetVersions.sizeBytes,
    ],
    name: "runtime_presentation_rights_evidence_asset_fk",
  }).onDelete("restrict"),
  unique("runtime_presentation_rights_evidence_exact_unique").on(
    table.id,
    table.assetVersionId,
    table.venueSlug,
    table.roomSlug,
...
export const runtimePresentationAdmissions = pgTable("runtime_presentation_admissions", {
  id: uuid("id").primaryKey().defaultRandom(),
  runtimePackageId: uuid("runtime_package_id").notNull(),
  runtimePackageContentDigest: varchar("runtime_package_content_digest", { length: 64 }).notNull(),
  venueSlug: varchar("venue_slug", { length: 100 }).notNull(),
  roomSlug: varchar("room_slug", { length: 100 }).notNull(),
  runtimeManifestDigest: varchar("runtime_manifest_digest", { length: 64 }).notNull(),
  reviewedProfileId: varchar("reviewed_profile_id", { length: 120 }).notNull(),
  reviewedProfileManifestFingerprint: varchar("reviewed_profile_manifest_fingerprint", { length: 64 }).notNull(),
  runtimeQaRecordId: uuid("runtime_qa_record_id").notNull(),
  runtimeQaRecordKey: varchar("runtime_qa_record_key", { length: 120 }).notNull(),
  runtimeQaRecordDigest: varchar("runtime_qa_record_digest", { length: 64 }).notNull(),
  runtimeQaDecision: varchar("runtime_qa_decision", { length: 40 })
    .$type<"approved_internal_preview" | "approved_public">()
    .notNull(),
  runtimeQaReviewedBy: uuid("runtime_qa_reviewed_by").notNull(),
  runtimeQaReviewedAt: timestamp("runtime_qa_reviewed_at", { withTimezone: true }).notNull(),
  runtimeTransformArtifactRowId: uuid("runtime_transform_artifact_row_id").notNull(),
  runtimeTransformArtifactId: varchar("runtime_transform_artifact_id", { length: 120 }).notNull(),
  runtimeTransformArtifactDigest: varchar("runtime_transform_artifact_digest", { length: 64 }).notNull(),
  sceneAuthorityArtifactRowId: uuid("scene_authority_artifact_row_id").notNull(),
  sceneAuthorityArtifactKind: varchar("scene_authority_artifact_kind", { length: 50 })
    .$type<"scene_authority_map_v0">()
    .notNull(),
  sceneAuthorityArtifactId: varchar("scene_authority_artifact_id", { length: 160 }).notNull(),
  sceneAuthorityMapDigest: varchar("scene_authority_map_digest", { length: 64 }).notNull(),
  rightsEvidenceDigest: varchar("rights_evidence_digest", { length: 64 }).notNull(),
  memberCount: integer("member_count").notNull(),
  decision: varchar("decision", { length: 20 }).$type<"approved" | "rejected">().notNull(),
  admissionDigest: varchar("admission_digest", { length: 64 }).notNull(),
  admissionBody: jsonb("admission_body").$type<Record<string, unknown>>().notNull(),
  reviewedBy: uuid("reviewed_by").notNull().references(() => users.id, { onDelete: "restrict" }),
  reviewedAt: timestamp("reviewed_at", { withTimezone: true }).notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
  foreignKey({
    columns: [
      ...
      table.venueSlug,
      table.roomSlug,
      table.runtimePackageContentDigest,
    ],
    foreignColumns: [
      runtimePackages.id,
      runtimePackages.venueSlug,
      runtimePackages.roomSlug,
      runtimePackages.contentDigest,
    ],
    name: "runtime_presentation_admissions_package_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.runtimeQaRecordId,
      table.runtimePackageId,
      table.venueSlug,
      table.roomSlug,
      table.runtimeQaRecordKey,
      table.runtimeQaRecordDigest,
      table.runtimeQaDecision,
      table.runtimeQaReviewedBy,
      table.runtimeQaReviewedAt,
    ],
    foreignColumns: [
      runtimeQaRecords.id,
      runtimeQaRecords.runtimePackageId,
      runtimeQaRecords.venueSlug,
      runtimeQaRecords.roomSlug,
      runtimeQaRecords.recordId,
      runtimeQaRecords.recordDigest,
      runtimeQaRecords.publicExposureDecision,
      runtimeQaRecords.reviewedBy,
      runtimeQaRecords.reviewedAt,
    ],
    name: "runtime_presentation_admissions_qa_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.runtimeTransformArtifactRowId,
      table.runtimePackageId,
      table.venueSlug,
      table.roomSlug,
      table.runtimeTransformArtifactId,
      table.runtimeTransformArtifactDigest,
    ],
    foreignColumns: [
      runtimeTransformArtifacts.id,
      runtimeTransformArtifacts.runtimePackageId,
      runtimeTransformArtifacts.venueSlug,
      runtimeTransformArtifacts.roomSlug,
      runtimeTransformArtifacts.transformArtifactId,
      runtimeTransformArtifacts.artifactDigest,
    ],
    name: "runtime_presentation_admissions_transform_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.sceneAuthorityArtifactRowId,
      table.venueSlug,
      table.sceneAuthorityArtifactKind,
      table.sceneAuthorityArtifactId,
      table.sceneAuthorityMapDigest,
    ],
    foreignColumns: [
      reconstructionReviewEvidenceArtifacts.id,
      reconstructionReviewEvidenceArtifacts.venueSlug,
      reconstructionReviewEvidenceArtifacts.artifactKind,
      reconstructionReviewEvidenceArtifacts.artifactId,
      reconstructionReviewEvidenceArtifacts.artifactDigest,
    ],
    name: "runtime_presentation_admissions_scene_authority_fk",
  }).onDelete("restrict"),
  unique("runtime_presentation_admissions_member_scope_unique").on(
    table.id,
    table.runtimePackageId,
    table.runtimePackageContentDigest,
    table.venueSlug,
  ...
  unique("runtime_presentation_admissions_exact_binding_unique").on(
    table.id,
    table.runtimePackageId,
    table.runtimePackageContentDigest,
    table.venueSlug,
    table.roomSlug,
    table.runtimeManifestDigest,
    table.runtimeQaRecordId,
    table.runtimeTransformArtifactRowId,
    table.decision,
    table.reviewedAt,
  ...
  index("runtime_presentation_admissions_package_idx").on(
    table.runtimePackageId,
    table.runtimePackageContentDigest,
    table.reviewedAt,
    table.id,
...
export const runtimePresentationAdmissionMembers = pgTable("runtime_presentation_admission_members", {
  ...
  sizeBytes: bigint("size_bytes", { mode: "number" }).notNull(),
  storageKeySha256: varchar("storage_key_sha256", { length: 64 }).notNull(),
  rightsEvidenceRowId: uuid("rights_evidence_row_id").notNull(),
  rightsEvidenceDigest: varchar("rights_evidence_digest", { length: 64 }).notNull(),
  rightsDecision: varchar("rights_decision", { length: 20 }).$type<"approved" | "rejected">().notNull(),
  rightsReviewedBy: uuid("rights_reviewed_by").notNull().references(() => users.id, { onDelete: "restrict" }),
  rightsReviewedAt: timestamp("rights_reviewed_at", { withTimezone: true }).notNull(),
}, (table) => [
  primaryKey({
    columns: [table.admissionId, table.memberIndex],
    name: "runtime_presentation_admission_members_pkey",
  }),
  foreignKey({
    columns: [
      table.admissionId,
    ...
    foreignColumns: [
      runtimePresentationAdmissions.id,
      runtimePresentationAdmissions.runtimePackageId,
      runtimePresentationAdmissions.runtimePackageContentDigest,
      runtimePresentationAdmissions.venueSlug,
      runtimePresentationAdmissions.roomSlug,
    ],
    name: "runtime_presentation_admission_members_admission_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.assetVersionId,
    ...
    foreignColumns: [
      ...
      assetVersions.venueSlug,
      assetVersions.roomSlug,
      assetVersions.fileName,
      assetVersions.fileExt,
      assetVersions.mimeType,
      assetVersions.sha256,
      assetVersions.sizeBytes,
    ],
    name: "runtime_presentation_admission_members_asset_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.rightsEvidenceRowId,
      table.assetVersionId,
      table.venueSlug,
      table.roomSlug,
      table.sha256,
      table.sizeBytes,
      table.rightsEvidenceDigest,
      table.rightsDecision,
      table.rightsReviewedBy,
      table.rightsReviewedAt,
    ],
    foreignColumns: [
      runtimePresentationRightsEvidence.id,
      runtimePresentationRightsEvidence.assetVersionId,
      runtimePresentationRightsEvidence.venueSlug,
      runtimePresentationRightsEvidence.roomSlug,
      runtimePresentationRightsEvidence.assetSha256,
    ...
    name: "runtime_presentation_admission_members_rights_fk",
  }).onDelete("restrict"),
  index("runtime_presentation_admission_members_asset_idx").on(table.assetVersionId),
]);
...
export const phaseLayoutSnapshots = pgTable("phase_layout_snapshots", {
  ...
  runtimeBinding: jsonb("runtime_binding").$type<PhaseLayoutRuntimeBinding | null>(),
  runtimePresentationAdmissionId: uuid("runtime_presentation_admission_id"),
  runtimePresentationAdmissionDecision: varchar("runtime_presentation_admission_decision", { length: 20 })
    .$type<"approved">(),
  runtimePresentationAdmissionReviewedAt: timestamp("runtime_presentation_admission_reviewed_at", { withTimezone: true }),
  runtimePresentationAdmissionDigest: varchar("runtime_presentation_admission_digest", { length: 64 }),
  runtimePackageId: uuid("runtime_package_id"),
  runtimePackageContentDigest: varchar("runtime_package_content_digest", { length: 64 }),
  runtimeVenueSlug: varchar("runtime_venue_slug", { length: 100 }),
  runtimeRoomSlug: varchar("runtime_room_slug", { length: 100 }),
  runtimeManifestDigest: varchar("runtime_manifest_digest", { length: 64 }),
  runtimeReviewedProfileId: varchar("runtime_reviewed_profile_id", { length: 120 }),
  runtimeReviewedProfileFingerprint: varchar("runtime_reviewed_profile_fingerprint", { length: 64 }),
  runtimeRightsEvidenceDigest: varchar("runtime_rights_evidence_digest", { length: 64 }),
  runtimeSceneAuthorityMapDigest: varchar("runtime_scene_authority_map_digest", { length: 64 }),
  runtimeQaRecordId: uuid("runtime_qa_record_id"),
  runtimeQaRecordKey: varchar("runtime_qa_record_key", { length: 120 }),
  runtimeQaRecordDigest: varchar("runtime_qa_record_digest", { length: 64 }),
  runtimeQaDecision: varchar("runtime_qa_decision", { length: 40 })
    .$type<"approved_internal_preview" | "approved_public">(),
  runtimeQaReviewedBy: uuid("runtime_qa_reviewed_by"),
  runtimeQaReviewedAt: timestamp("runtime_qa_reviewed_at", { withTimezone: true }),
  runtimeTransformArtifactRowId: uuid("runtime_transform_artifact_row_id"),
  runtimeTransformArtifactId: varchar("runtime_transform_artifact_id", { length: 120 }),
  runtimeTransformArtifactDigest: varchar("runtime_transform_artifact_digest", { length: 64 }),
  runtimeExecutionActivationId: uuid("runtime_execution_activation_id"),
  runtimeExecutionActivationPredicateDigest: varchar("runtime_execution_activation_predicate_digest", { length: 64 }),
  runtimeExecutionActivationPayloadSha256: varchar("runtime_execution_activation_payload_sha256", { length: 64 }),
  runtimeExecutionActivationEnvelopeSha256: varchar("runtime_execution_activation_envelope_sha256", { length: 64 }),
  runtimeExecutionActivationExpiresAt: timestamp("runtime_execution_activation_expires_at", { withTimezone: true }),
  runtimeExecutionKeyPolicyId: uuid("runtime_execution_key_policy_id"),
  runtimeExecutionKeyPolicyDigest: varchar("runtime_execution_key_policy_digest", { length: 64 }),
  runtimeExecutionKeyId: varchar("runtime_execution_key_id", { length: 160 }),
  runtimeExecutionPublicKeyFingerprint: varchar("runtime_execution_public_key_fingerprint", { length: 64 }),
  status: varchar("status", { length: 30 }).notNull().default("draft"),
  ...
  foreignKey({
    columns: [
      table.runtimePackageId,
      table.runtimeVenueSlug,
      table.runtimeRoomSlug,
      table.runtimePackageContentDigest,
    ],
    foreignColumns: [
      runtimePackages.id,
    ...
    name: "phase_layout_snapshots_runtime_package_scope_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.runtimeQaRecordId,
    ...
    name: "phase_layout_snapshots_runtime_qa_exact_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.runtimeTransformArtifactRowId,
    ...
    name: "phase_layout_snapshots_runtime_transform_exact_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.runtimePresentationAdmissionId,
      table.runtimePackageId,
      table.runtimePackageContentDigest,
      table.runtimeVenueSlug,
      table.runtimeRoomSlug,
      ...
      table.runtimeQaRecordId,
      table.runtimeTransformArtifactRowId,
      table.runtimePresentationAdmissionDecision,
      table.runtimePresentationAdmissionReviewedAt,
      table.runtimePresentationAdmissionDigest,
    ],
    foreignColumns: [
      runtimePresentationAdmissions.id,
      runtimePresentationAdmissions.runtimePackageId,
      ...
      runtimePresentationAdmissions.venueSlug,
      runtimePresentationAdmissions.roomSlug,
      runtimePresentationAdmissions.runtimeManifestDigest,
      runtimePresentationAdmissions.runtimeQaRecordId,
      runtimePresentationAdmissions.runtimeTransformArtifactRowId,
      runtimePresentationAdmissions.decision,
      runtimePresentationAdmissions.reviewedAt,
      runtimePresentationAdmissions.admissionDigest,
    ],
    name: "phase_layout_snapshots_runtime_admission_exact_fk",
  }).onDelete("restrict"),
  foreignKey({
    columns: [
      table.runtimeExecutionActivationId,
      table.runtimeExecutionActivationPredicateDigest,
      table.runtimeExecutionActivationPayloadSha256,
      table.runtimeExecutionActivationEnvelopeSha256,
      table.runtimeExecutionActivationExpiresAt,
      table.runtimeExecutionKeyPolicyId,
      table.runtimeExecutionKeyPolicyDigest,
      table.runtimeExecutionKeyId,
      table.runtimeExecutionPublicKeyFingerprint,
    ],
    foreignColumns: [
      runtimeExecutionActivations.id,
      runtimeExecutionActivations.predicateDigest,
      runtimeExecutionActivations.payloadSha256,
      runtimeExecutionActivations.envelopeSha256,
      runtimeExecutionActivations.expiresAt,
      runtimeExecutionActivations.keyPolicyId,
      runtimeExecutionActivations.keyPolicyDigest,
      runtimeExecutionActivations.keyId,
      runtimeExecutionActivations.publicKeyFingerprint,
    ],
    name: "phase_layout_snapshots_runtime_execution_activation_fk",
  }).onDelete("restrict"),
]);
...
// Purpose-scoped runtime execution authority. These records are distinct from
// Foundry release attestations: a release signature never implies viewer
// execution authority. Migration 0064 makes every record append-only.
export const runtimeExecutionKeyPolicies = pgTable("runtime_execution_key_policies", {
  id: uuid("id").primaryKey(),
  purpose: varchar("purpose", { length: 80 })
    .$type<"historical_runtime_execution_activation">()
    .notNull(),
  algorithm: varchar("algorithm", { length: 20 }).$type<"ed25519">().notNull(),
  ...
  policyBody: jsonb("policy_body").$type<HistoricalRuntimeExecutionKeyPolicy>().notNull(),
  registeredBy: uuid("registered_by").notNull().references(() => users.id, { onDelete: "restrict" }),
  registeredAt: timestamp("registered_at", { withTimezone: true }).defaultNow().notNull(),
  effectiveAt: timestamp("effective_at", { withTimezone: true }).notNull(),
  expiresAt: timestamp("expires_at", { withTimezone: true }).notNull(),
}, (table) => [
  unique("runtime_execution_key_policies_exact_unique").on(
    table.id,
    table.policyDigest,
    table.keyId,
    table.publicKeyFingerprint,
  ),
  index("runtime_execution_key_policies_current_idx").on(
    table.keyId,
    table.effectiveAt,
    table.expiresAt,
  ),
...
export const runtimeExecutionKeyPolicyRevocations = pgTable("runtime_execution_key_policy_revocations", {
  policyId: uuid("policy_id").primaryKey().references(() => runtimeExecutionKeyPolicies.id, { onDelete: "restrict" }),
  revocationDigest: varchar("revocation_digest", { length: 64 }).notNull(),
  revocationBody: jsonb("revocation_body").$type<Record<string, unknown>>().notNull(),
  reason: varchar("reason", { length: 240 }).notNull(),
  revokedBy: uuid("revoked_by").notNull().references(() => users.id, { onDelete: "restrict" }),
  revokedAt: timestamp("revoked_at", { withTimezone: true }).defaultNow().notNull(),
});

// One immutable server-issued statement. The signing service receives only
// the exact canonical bytes stored here; the API does not hold a private key.
export const runtimeExecutionActivationDrafts = pgTable("runtime_execution_activation_drafts", {
  id: uuid("id").primaryKey(),
  eventId: uuid("event_id").notNull().references(() => events.id, { onDelete: "restrict" }),
  phaseId: uuid("phase_id").notNull().references(() => eventPhases.id, { onDelete: "restrict" }),
  configurationId: uuid("configuration_id").notNull().references(() => configurations.id, { onDelete: "restrict" }),
  canonicalSnapshotId: uuid("canonical_snapshot_id").notNull().references(() => canonicalLayoutSnapshots.id, { onDelete: "restrict" }),
  snapshotHash: varchar("snapshot_hash", { length: 64 }).notNull(),
  proofDigest: varchar("proof_digest", { length: 64 }).notNull(),
  tenantVenueId: uuid("tenant_venue_id").notNull().references(() => venues.id, { onDelete: "restrict" }),
  ...
  spaceSlug: varchar("space_slug", { length: 100 }).notNull(),
  presentationAdmissionId: uuid("presentation_admission_id").notNull().references(() => runtimePresentationAdmissions.id, { onDelete: "restrict" }),
  presentationAdmissionDigest: varchar("presentation_admission_digest", { length: 64 }).notNull(),
  runtimePackageId: uuid("runtime_package_id").notNull().references(() => runtimePackages.id, { onDelete: "restrict" }),
  runtimePackageContentDigest: varchar("runtime_package_content_digest", { length: 64 }).notNull(),
  runtimeManifestDigest: varchar("runtime_manifest_digest", { length: 64 }).notNull(),
  compositionDigest: varchar("composition_digest", { length: 64 }).notNull(),
  memberCount: integer("member_count").notNull(),
  keyPolicyId: uuid("key_policy_id").notNull(),
  keyPolicyDigest: varchar("key_policy_digest", { length: 64 }).notNull(),
  keyId: varchar("key_id", { length: 160 }).notNull(),
  publicKeyFingerprint: varchar("public_key_fingerprint", { length: 64 }).notNull(),
  ...
  predicateDigest: varchar("predicate_digest", { length: 64 }).notNull(),
  payloadSha256: varchar("payload_sha256", { length: 64 }).notNull(),
  statement: jsonb("statement").$type<HistoricalRuntimeExecutionStatement>().notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
  foreignKey({
    columns: [
      table.keyPolicyId,
      table.keyPolicyDigest,
      table.keyId,
      table.publicKeyFingerprint,
    ],
    foreignColumns: [
      runtimeExecutionKeyPolicies.id,
      runtimeExecutionKeyPolicies.policyDigest,
      runtimeExecutionKeyPolicies.keyId,
      runtimeExecutionKeyPolicies.publicKeyFingerprint,
    ],
    name: "runtime_execution_activation_drafts_key_policy_fk",
  }).onDelete("restrict"),
  unique("runtime_execution_activation_drafts_nonce_unique").on(table.nonce),
  unique("runtime_execution_activation_drafts_exact_unique").on(
    table.id,
    table.predicateDigest,
    table.payloadSha256,
    table.expiresAt,
    table.keyPolicyId,
    table.keyPolicyDigest,
  ...
  index("runtime_execution_activation_drafts_target_idx").on(
    table.eventId,
    table.phaseId,
    table.configurationId,
    table.canonicalSnapshotId,
  ),
]);

export const runtimeExecutionActivationDraftMembers = pgTable("runtime_execution_activation_draft_members", {
  activationId: uuid("activation_id").notNull().references(() => runtimeExecutionActivationDrafts.id, { onDelete: "restrict" }),
  memberIndex: integer("member_index").notNull(),
  admissionId: uuid("admission_id").notNull(),
  ...
  rightsReviewedAt: timestamp("rights_reviewed_at", { withTimezone: true }).notNull(),
  storageKeySha256: varchar("storage_key_sha256", { length: 64 }).notNull(),
  privateBucketSha256: varchar("private_bucket_sha256", { length: 64 }).notNull(),
  storageVersion: varchar("storage_version", { length: 200 }).notNull(),
  storageEtag: varchar("storage_etag", { length: 200 }).notNull(),
  objectReceiptDigest: varchar("object_receipt_digest", { length: 64 }).notNull(),
  receiptBody: jsonb("receipt_body").$type<HistoricalRuntimeExecutionObjectReceipt>().notNull(),
}, (table) => [
  primaryKey({
    columns: [table.activationId, table.memberIndex],
    name: "runtime_execution_activation_draft_members_pkey",
  }),
  foreignKey({
    columns: [table.admissionId, table.memberIndex],
    foreignColumns: [
      runtimePresentationAdmissionMembers.admissionId,
      runtimePresentationAdmissionMembers.memberIndex,
    ],
    name: "runtime_execution_activation_draft_members_admission_member_fk",
  }).onDelete("restrict"),
  unique("runtime_execution_activation_draft_members_exact_unique").on(
    table.activationId,
    table.memberIndex,
    table.admissionId,
    table.assetVersionId,
    table.sha256,
    table.sizeBytes,
    table.storageKeySha256,
    table.privateBucketSha256,
    table.storageVersion,
    table.storageEtag,
    table.objectReceiptDigest,
  ),
]);

export const runtimeExecutionActivations = pgTable("runtime_execution_activations", {
  id: uuid("id").primaryKey(),
  predicateDigest: varchar("predicate_digest", { length: 64 }).notNull(),
  payloadSha256: varchar("payload_sha256", { length: 64 }).notNull(),
  envelopeSha256: varchar("envelope_sha256", { length: 64 }).notNull(),
  envelope: jsonb("envelope").$type<ReconstructionDsseEnvelope>().notNull(),
  keyPolicyId: uuid("key_policy_id").notNull(),
  keyPolicyDigest: varchar("key_policy_digest", { length: 64 }).notNull(),
  keyId: varchar("key_id", { length: 160 }).notNull(),
  publicKeyFingerprint: varchar("public_key_fingerprint", { length: 64 }).notNull(),
  ...
  foreignKey({
    ...
    foreignColumns: [
      runtimeExecutionActivationDrafts.id,
      runtimeExecutionActivationDrafts.predicateDigest,
      runtimeExecutionActivationDrafts.payloadSha256,
      runtimeExecutionActivationDrafts.expiresAt,
      runtimeExecutionActivationDrafts.keyPolicyId,
      runtimeExecutionActivationDrafts.keyPolicyDigest,
      runtimeExecutionActivationDrafts.keyId,
      runtimeExecutionActivationDrafts.publicKeyFingerprint,
    ],
    name: "runtime_execution_activations_draft_fk",
  }).onDelete("restrict"),
  unique("runtime_execution_activations_exact_unique").on(
    table.id,
    table.predicateDigest,
    table.payloadSha256,
  ...
  index("runtime_execution_activations_current_idx").on(table.expiresAt, table.id),
]);

export const runtimeExecutionActivationMembers = pgTable("runtime_execution_activation_members", {
  activationId: uuid("activation_id").notNull().references(() => runtimeExecutionActivations.id, { onDelete: "restrict" }),
  memberIndex: integer("member_index").notNull(),
  admissionId: uuid("admission_id").notNull(),
  assetVersionId: uuid("asset_version_id").notNull(),
  sha256: varchar("sha256", { length: 64 }).notNull(),
  ...
  primaryKey({
    columns: [table.activationId, table.memberIndex],
    name: "runtime_execution_activation_members_pkey",
  }),
  foreignKey({
    columns: [
      table.activationId,
    ...
    foreignColumns: [
      runtimeExecutionActivationDraftMembers.activationId,
      runtimeExecutionActivationDraftMembers.memberIndex,
      runtimeExecutionActivationDraftMembers.admissionId,
      runtimeExecutionActivationDraftMembers.assetVersionId,
      runtimeExecutionActivationDraftMembers.sha256,
      runtimeExecutionActivationDraftMembers.sizeBytes,
      runtimeExecutionActivationDraftMembers.storageKeySha256,
      runtimeExecutionActivationDraftMembers.privateBucketSha256,
      runtimeExecutionActivationDraftMembers.storageVersion,
      runtimeExecutionActivationDraftMembers.storageEtag,
      runtimeExecutionActivationDraftMembers.objectReceiptDigest,
    ],
    name: "runtime_execution_activation_members_draft_member_fk",
  }).onDelete("restrict"),
]);

export const runtimeExecutionActivationRevocations = pgTable("runtime_execution_activation_revocations", {
  activationId: uuid("activation_id").primaryKey().references(() => runtimeExecutionActivations.id, { onDelete: "restrict" }),
  revocationDigest: varchar("revocation_digest", { length: 64 }).notNull(),
  revocationBody: jsonb("revocation_body").$type<Record<string, unknown>>().notNull(),
  reason: varchar("reason", { length: 240 }).notNull(),
  revokedBy: uuid("revoked_by").notNull().references(() => users.id, { onDelete: "restrict" }),
