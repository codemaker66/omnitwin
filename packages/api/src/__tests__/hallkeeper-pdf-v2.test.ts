...
describe("generateSheetPdfV2 — baseline", () => {
  it("renders frozen footprints with actual chair counts instead of a generation placeholder", async () => {
    ...
    // Plain orientation wording: "+X right, +Z down" is the renderer's
    // vocabulary and means nothing on a hallkeeper's printout.
    expect(text).toContain("Plan drawn looking down on the room");
    expect(text).not.toContain("+X right");
...
describe("PDF times are pinned to the venue's timezone", () => {
  ...
  it("names the reason when the set-up deadline is not derivable", async () => {
    // setupBy is null when the venue records no turnaround rule and no
    // earlier phase exists. The sheet prints the REASON — never a constant.
    const text = pdfPages(await generateSheetPdfV2({
      ...BASE_SHEET,
      timing: { eventStart: "2026-06-15T17:30:00.000Z", setupBy: null, bufferMinutes: null },
      venue: { ...BASE_SHEET.venue, timezone: "Europe/London" },
    })).flat().map((line) => line.text).join(" | ");
