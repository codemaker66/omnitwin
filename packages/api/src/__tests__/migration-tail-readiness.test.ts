...
const EXPECTED_TAIL = [
  ...
  "0064_venue_inventory",
  "0065_inventory_reservations",
] as const;
...
describe("migration tail rollout readiness", () => {
  it("registers reservation decision tables and immutable history in migration 0065", async () => {
    const sql = await readMigration("0065_inventory_reservations");
  ...
  it("discovers inventory tables, checks and indexes from migration 0064's unquoted identifiers", async () => {
    const migrations = [{ sql: await readMigration("0064_venue_inventory") }];
