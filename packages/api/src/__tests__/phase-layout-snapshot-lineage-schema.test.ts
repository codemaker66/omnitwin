...
describe("phase layout snapshot lineage migration", () => {
  ...
  it("keeps 0059 → 0060 contiguous and registers the immutable successor", async () => {
    ...
    expect(journal.entries.find((entry) => entry.tag === "0063_phase_layout_snapshot_immutability")).toEqual(expect.objectContaining({
      idx: 61,
      tag: "0063_phase_layout_snapshot_immutability",
    }));
  });

  it("blocks mutation only after a phase layout snapshot is frozen", async () => {
    const migration = await readFile(
      resolve("drizzle/0063_phase_layout_snapshot_immutability.sql"),
      "utf-8",
    );
