...
export const FOUNDRY_SOURCE_READINESS_MAP_V5 =
  "omnitwin.foundry.source-readiness-map.v5";
export const FOUNDRY_SOURCE_READINESS_MAP_V5_DIGEST_DOMAIN =
...
const LaneCountsSchema = z
  .object({
    ...
    outsideSourceFactsV5Count: z.number().int().nonnegative(),
...
const BlockedReasonSchema = z
  .object({
    ...
    message: z.literal(
      "The source set includes an XGRIDS XBIN candidate, so Source Readiness Map V5 exposes no partial file or lane evidence.",
    ),
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_SOURCE_READINESS_MAP_V5),
...
export const FoundrySourceReadinessMapV5Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundrySourceReadinessMapV5 = z.infer<
  typeof FoundrySourceReadinessMapV5Schema
>;
...
const GENERIC_GAP_DETAILS = {
  ...
  OUTSIDE_SOURCE_FACTS_V5: {
    label: "Outside Source Facts V5",
    reason: "The receipt has candidate source families, but Source Facts V5 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: {
    ...
    reason: "Source Facts V5 targeted the file but did not establish its bounded format facts.",
...
function validateFileEvidenceState(
  ...
  if (file.unknowns.length > 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["unknowns"],
      message: "files outside Source Facts V5 cannot claim format unknowns",
    });
  }
...
function statusDecisiveTest(status: FileStatus): string | null {
  switch (status) {
    ...
    case "outside_source_facts_v5":
      return GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V5.decisiveNextTest;
...
function countsForFiles(files: readonly ReadinessFile[]): z.infer<typeof LaneCountsSchema> {
  return {
    ...
    outsideSourceFactsV5Count: files.filter(
      (file) => file.status === "outside_source_facts_v5",
    ).length,
...
function gapForStatus(
  ...
  const statusByCode: Partial<Record<keyof typeof GENERIC_GAP_DETAILS, FileStatus>> = {
    ...
    OUTSIDE_SOURCE_FACTS_V5: "outside_source_facts_v5",
...
function summaryForAvailable(
  ...
  return {
    ...
    outsideSourceFactsV5Count: counts.outsideSourceFactsV5Count,
...
function issueArtifact(
  ...
  return FoundrySourceReadinessMapV5Schema.parse(issued);
}
...
function expectedTarget(
  file: FoundryUniversalIntakeFile,
  refinedAsset?: UniversalSourceFactsV5Asset,
): ExpectedTarget | null {
  const magic = file.inspection.magicHex;
...
function assetIdentity(asset: UniversalSourceFactsV5Asset): {
...
function assertAvailableSourceFactsCoverage(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV5, { readonly state: "available" }>,
): Map<string, UniversalSourceFactsV5Asset> {
    ...
    if (target === null) {
      if (asset !== undefined) {
        throw new FoundryIntegrityError(
          ...
          `Source Facts targets ${file.path}, which is outside its V5 target rules.`,
        );
      }
      continue;
    }
...
function sortedUnknowns(asset: UniversalSourceFactsV5Asset): UnknownFact[] {
...
function fileFromReceipt(
  file: FoundryUniversalIntakeFile,
  asset: UniversalSourceFactsV5Asset | undefined,
): ReadinessFile {
  if (asset !== undefined) {
    ...
    decisiveNextTests: [
      status === "ambiguous_format"
        ...
        : status === "unclassified_format"
          ...
          : GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V5.decisiveNextTest,
    ],
  });
}
...
function assertExactXbinBinding(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV5, { readonly state: "unavailable" }>,
): void {
...
export interface CompileFoundrySourceReadinessMapV5Input {
 ...
 */
export function compileFoundrySourceReadinessMapV5(
  input: CompileFoundrySourceReadinessMapV5Input,
): FoundrySourceReadinessMapV5 {
  const receipt = FoundryUniversalIntakeReceiptSchema.parse(input.receipt);
  const sourceFacts = FoundryUniversalSourceFactsV5Schema.parse(input.sourceFacts);
  ...
  if (xbinSources.length > 0) {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_SOURCE_READINESS_MAP_V5,
...
export function serializeFoundrySourceReadinessMapV5(
  value: FoundrySourceReadinessMapV5,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundrySourceReadinessMapV5Schema.parse(value)),
  );
}
