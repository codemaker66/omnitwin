...
import {
  ...
  type FoundryUniversalSourceFactsV4,
  type UniversalSourceFactsV4Asset,
} from "./source-facts-v4.js";
import { FOUNDRY_XBIN_OFFICIAL_EXPORT_NEXT_ACTION } from "./source-facts.js";

export const FOUNDRY_SOURCE_READINESS_MAP_V4 =
  "omnitwin.foundry.source-readiness-map.v4";
export const FOUNDRY_SOURCE_READINESS_MAP_V4_DIGEST_DOMAIN =
  "VENVIEWER_FOUNDRY_SOURCE_READINESS_MAP_V4";
...
const LaneCountsSchema = z
  .object({
    ...
    outsideSourceFactsV4Count: z.number().int().nonnegative(),
...
const BlockedReasonSchema = z
  .object({
    ...
    message: z.literal(
      "The source set includes an XGRIDS XBIN candidate, so Source Readiness Map V4 exposes no partial file or lane evidence.",
    ),
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_SOURCE_READINESS_MAP_V4),
...
export const FoundrySourceReadinessMapV4Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundrySourceReadinessMapV4 = z.infer<
  typeof FoundrySourceReadinessMapV4Schema
>;
...
const GENERIC_GAP_DETAILS = {
  ...
  OUTSIDE_SOURCE_FACTS_V4: {
    label: "Outside Source Facts V4",
    reason: "The receipt has candidate source families, but Source Facts V4 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: {
    ...
    reason: "Source Facts V4 targeted the file but did not establish its bounded format facts.",
...
function validateFileEvidenceState(
  ...
  if (file.unknowns.length > 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["unknowns"],
      message: "files outside Source Facts V4 cannot claim format unknowns",
    });
  }
...
function statusDecisiveTest(status: FileStatus): string | null {
  switch (status) {
    ...
    case "outside_source_facts_v4":
      return GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V4.decisiveNextTest;
...
function countsForFiles(files: readonly ReadinessFile[]): z.infer<typeof LaneCountsSchema> {
  return {
    ...
    outsideSourceFactsV4Count: files.filter(
      (file) => file.status === "outside_source_facts_v4",
    ).length,
...
function gapForStatus(
  ...
  const statusByCode: Partial<Record<keyof typeof GENERIC_GAP_DETAILS, FileStatus>> = {
    ...
    OUTSIDE_SOURCE_FACTS_V4: "outside_source_facts_v4",
...
function summaryForAvailable(
  ...
  return {
    ...
    outsideSourceFactsV4Count: counts.outsideSourceFactsV4Count,
...
function issueArtifact(
  ...
  return FoundrySourceReadinessMapV4Schema.parse(issued);
}
...
function expectedTarget(
  file: FoundryUniversalIntakeFile,
  refinedAsset?: UniversalSourceFactsV4Asset,
): ExpectedTarget | null {
  const magic = file.inspection.magicHex;
...
function assetIdentity(asset: UniversalSourceFactsV4Asset): {
...
function assertAvailableSourceFactsCoverage(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV4, { readonly state: "available" }>,
): Map<string, UniversalSourceFactsV4Asset> {
    ...
    if (target === null) {
      if (asset !== undefined) {
        throw new FoundryIntegrityError(
          ...
          `Source Facts targets ${file.path}, which is outside its V4 target rules.`,
        );
      }
      continue;
    }
...
function sortedUnknowns(asset: UniversalSourceFactsV4Asset): UnknownFact[] {
...
function fileFromReceipt(
  file: FoundryUniversalIntakeFile,
  asset: UniversalSourceFactsV4Asset | undefined,
): ReadinessFile {
  if (asset !== undefined) {
    ...
    decisiveNextTests: [
      status === "ambiguous_format"
        ...
        : status === "unclassified_format"
          ...
          : GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V4.decisiveNextTest,
    ],
  });
}
...
function assertExactXbinBinding(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV4, { readonly state: "unavailable" }>,
): void {
...
export interface CompileFoundrySourceReadinessMapV4Input {
 ...
 */
export function compileFoundrySourceReadinessMapV4(
  input: CompileFoundrySourceReadinessMapV4Input,
): FoundrySourceReadinessMapV4 {
  const receipt = FoundryUniversalIntakeReceiptSchema.parse(input.receipt);
  const sourceFacts = FoundryUniversalSourceFactsV4Schema.parse(input.sourceFacts);
  ...
  if (xbinSources.length > 0) {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_SOURCE_READINESS_MAP_V4,
...
export function serializeFoundrySourceReadinessMapV4(
  value: FoundrySourceReadinessMapV4,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundrySourceReadinessMapV4Schema.parse(value)),
  );
}
