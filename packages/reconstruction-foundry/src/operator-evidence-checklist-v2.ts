...
import {
  ...
  type FoundrySourceReadinessMapV2,
} from "./source-readiness-v2.js";

export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V2 =
  "omnitwin.foundry.operator-evidence-checklist.v2";
export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V2_DIGEST_DOMAIN =
  "VENVIEWER_FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V2";
...
const REQUEST_DEFINITIONS = {
  ...
  OUTSIDE_SOURCE_FACTS_V2: requestDefinition(
    "gap",
    "bounded_inspection",
    "high",
    "Outside Source Facts V2",
    "The receipt has candidate source families, but Source Facts V2 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: requestDefinition(
    ...
    "Source Facts V2 targeted the file but did not establish its bounded format facts.",
...
const GROUP_DESCRIPTORS = [
  {
    ...
    meaning:
      "Resolve format ambiguity, missing format identification, unsupported V2 coverage, or a bounded inspection failure before relying on deeper source facts.",
  },
  {
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V2),
...
export const FoundryOperatorEvidenceChecklistV2Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundryOperatorEvidenceChecklistV2 = z.infer<
  typeof FoundryOperatorEvidenceChecklistV2Schema
>;
...
function issueArtifact(
  ...
  return FoundryOperatorEvidenceChecklistV2Schema.parse({
    ...payload,
...
export interface CompileFoundryOperatorEvidenceChecklistV2Input {
  readonly readiness: unknown;
}

/**
 * Builds a deterministic, unperformed evidence-request checklist from only an
 * exact Source Readiness Map V2. It performs no evidence acquisition or work.
 */
export function compileFoundryOperatorEvidenceChecklistV2(
  input: CompileFoundryOperatorEvidenceChecklistV2Input,
): FoundryOperatorEvidenceChecklistV2 {
  const readiness = FoundrySourceReadinessMapV2Schema.parse(input.readiness);
  if (readiness.state === "blocked") {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V2,
...
export interface VerifyFoundryOperatorEvidenceChecklistV2Input {
  readonly readiness: unknown;
  readonly checklist: unknown;
}

export function verifyFoundryOperatorEvidenceChecklistV2(
  input: VerifyFoundryOperatorEvidenceChecklistV2Input,
): FoundryOperatorEvidenceChecklistV2 {
  const actual = FoundryOperatorEvidenceChecklistV2Schema.parse(
    input.checklist,
  );
  const expected = compileFoundryOperatorEvidenceChecklistV2({
    readiness: input.readiness,
  });
  if (
    serializeFoundryOperatorEvidenceChecklistV2(actual) !==
    serializeFoundryOperatorEvidenceChecklistV2(expected)
  ) {
    throw new FoundryIntegrityError(
...
export function serializeFoundryOperatorEvidenceChecklistV2(
  value: FoundryOperatorEvidenceChecklistV2,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundryOperatorEvidenceChecklistV2Schema.parse(value)),
  );
}
