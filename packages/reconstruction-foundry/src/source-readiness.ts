...
import {
  ...
  type UniversalSourceFactsAsset,
} from "./source-facts.js";

export const FOUNDRY_SOURCE_READINESS_MAP_V1 =
  "omnitwin.foundry.source-readiness-map.v1";
export const FOUNDRY_SOURCE_READINESS_MAP_DIGEST_DOMAIN =
...
const ReadinessFileSchema = z
  .object({
    ...
    format: z.enum(["e57", "glb", "gltf_json", "obj", "sog"]).nullable(),
...
const LaneCountsSchema = z
  .object({
    ...
    outsideSourceFactsV1Count: z.number().int().nonnegative(),
...
const BlockedReasonSchema = z
  .object({
    ...
    message: z.literal(
      "The source set includes an XGRIDS XBIN candidate, so Source Readiness Map V1 exposes no partial file or lane evidence.",
    ),
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_SOURCE_READINESS_MAP_V1),
...
export const FoundrySourceReadinessMapV1Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundrySourceReadinessMapV1 = z.infer<
  typeof FoundrySourceReadinessMapV1Schema
>;
...
const GENERIC_GAP_DETAILS = {
  ...
  OUTSIDE_SOURCE_FACTS_V1: {
    label: "Outside Source Facts V1",
    reason: "The receipt has candidate source families, but Source Facts V1 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: {
    ...
    reason: "Source Facts V1 targeted the file but did not establish its bounded format facts.",
...
function validateFileEvidenceState(
  ...
  if (file.unknowns.length > 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["unknowns"],
      message: "files outside Source Facts V1 cannot claim format unknowns",
    });
  }
...
function statusDecisiveTest(status: FileStatus): string | null {
  switch (status) {
    ...
    case "outside_source_facts_v1":
      return GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V1.decisiveNextTest;
...
function countsForFiles(files: readonly ReadinessFile[]): z.infer<typeof LaneCountsSchema> {
  return {
    ...
    outsideSourceFactsV1Count: files.filter(
      (file) => file.status === "outside_source_facts_v1",
    ).length,
...
function gapForStatus(
  ...
  const statusByCode: Partial<Record<keyof typeof GENERIC_GAP_DETAILS, FileStatus>> = {
    ...
    OUTSIDE_SOURCE_FACTS_V1: "outside_source_facts_v1",
...
function summaryForAvailable(
  ...
  return {
    ...
    outsideSourceFactsV1Count: counts.outsideSourceFactsV1Count,
...
function issueArtifact(
  ...
  return FoundrySourceReadinessMapV1Schema.parse(issued);
}
...
interface ExpectedTarget {
  readonly inputType: "generic_e57" | "matterport_e57" | "glb_gltf" | "obj" | "sog";
  readonly format: "e57" | "glb" | "gltf_json" | "obj" | "sog";
}
...
function assetIdentity(asset: UniversalSourceFactsAsset): {
...
function assertAvailableSourceFactsCoverage(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFacts, { readonly state: "available" }>,
): Map<string, UniversalSourceFactsAsset> {
    ...
    if (target === null) {
      if (asset !== undefined) {
        throw new FoundryIntegrityError(
          ...
          `Source Facts targets ${file.path}, which is outside its V1 target rules.`,
        );
      }
      continue;
    }
...
function sortedUnknowns(asset: UniversalSourceFactsAsset): UnknownFact[] {
...
function fileFromReceipt(
  file: FoundryUniversalIntakeFile,
  asset: UniversalSourceFactsAsset | undefined,
): ReadinessFile {
  if (asset !== undefined) {
    ...
    decisiveNextTests: [
      status === "ambiguous_format"
        ...
        : status === "unclassified_format"
          ...
          : GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V1.decisiveNextTest,
    ],
  });
}
...
function assertExactXbinBinding(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFacts, { readonly state: "unavailable" }>,
): void {
...
export interface CompileFoundrySourceReadinessMapV1Input {
 ...
 */
export function compileFoundrySourceReadinessMapV1(
  input: CompileFoundrySourceReadinessMapV1Input,
): FoundrySourceReadinessMapV1 {
  const receipt = FoundryUniversalIntakeReceiptSchema.parse(input.receipt);
  const sourceFacts = FoundryUniversalSourceFactsSchema.parse(input.sourceFacts);
  ...
  if (xbinSources.length > 0) {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_SOURCE_READINESS_MAP_V1,
...
export function serializeFoundrySourceReadinessMapV1(
  value: FoundrySourceReadinessMapV1,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundrySourceReadinessMapV1Schema.parse(value)),
  );
}
