...
import {
  ...
  type FoundryUniversalSourceFactsV3,
  type UniversalSourceFactsV3Asset,
} from "./source-facts-v3.js";
import { FOUNDRY_XBIN_OFFICIAL_EXPORT_NEXT_ACTION } from "./source-facts.js";

export const FOUNDRY_SOURCE_READINESS_MAP_V3 =
  "omnitwin.foundry.source-readiness-map.v3";
export const FOUNDRY_SOURCE_READINESS_MAP_V3_DIGEST_DOMAIN =
  "VENVIEWER_FOUNDRY_SOURCE_READINESS_MAP_V3";
...
const ReadinessFileSchema = z
  .object({
    ...
    format: z
      .enum(["e57", "gaussian_ply", "glb", "gltf_json", "obj", "sog", "spz"])
      .nullable(),
...
const LaneCountsSchema = z
  .object({
    ...
    outsideSourceFactsV3Count: z.number().int().nonnegative(),
...
const BlockedReasonSchema = z
  .object({
    ...
    message: z.literal(
      "The source set includes an XGRIDS XBIN candidate, so Source Readiness Map V3 exposes no partial file or lane evidence.",
    ),
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_SOURCE_READINESS_MAP_V3),
...
export const FoundrySourceReadinessMapV3Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundrySourceReadinessMapV3 = z.infer<
  typeof FoundrySourceReadinessMapV3Schema
>;
...
const GENERIC_GAP_DETAILS = {
  ...
  OUTSIDE_SOURCE_FACTS_V3: {
    label: "Outside Source Facts V3",
    reason: "The receipt has candidate source families, but Source Facts V3 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: {
    ...
    reason: "Source Facts V3 targeted the file but did not establish its bounded format facts.",
...
function validateFileEvidenceState(
  ...
  if (factsStatus) {
    ...
    if (file.inputType !== null && !detectionCarriesInputType && !establishedGaussianRefinement) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["detection", "candidates"],
        message: "Source Facts input type must remain present in receipt detection evidence or be an established Gaussian refinement of an ordinary PLY candidate",
      });
    }
  ...
  if (file.unknowns.length > 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["unknowns"],
      message: "files outside Source Facts V3 cannot claim format unknowns",
    });
  }
...
function statusDecisiveTest(status: FileStatus): string | null {
  switch (status) {
    ...
    case "outside_source_facts_v3":
      return GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V3.decisiveNextTest;
...
function countsForFiles(files: readonly ReadinessFile[]): z.infer<typeof LaneCountsSchema> {
  return {
    ...
    outsideSourceFactsV3Count: files.filter(
      (file) => file.status === "outside_source_facts_v3",
    ).length,
...
function gapForStatus(
  ...
  const statusByCode: Partial<Record<keyof typeof GENERIC_GAP_DETAILS, FileStatus>> = {
    ...
    OUTSIDE_SOURCE_FACTS_V3: "outside_source_facts_v3",
...
function summaryForAvailable(
  ...
  return {
    ...
    outsideSourceFactsV3Count: counts.outsideSourceFactsV3Count,
...
function issueArtifact(
  ...
  return FoundrySourceReadinessMapV3Schema.parse(issued);
}
...
function expectedTarget(
  ...
  refinedAsset?: UniversalSourceFactsV3Asset,
): ExpectedTarget | null {
...
function assetIdentity(asset: UniversalSourceFactsV3Asset): {
...
function assertAvailableSourceFactsCoverage(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV3, { readonly state: "available" }>,
): Map<string, UniversalSourceFactsV3Asset> {
    ...
    if (target === null) {
      if (asset !== undefined) {
        throw new FoundryIntegrityError(
          ...
          `Source Facts targets ${file.path}, which is outside its V3 target rules.`,
        );
      }
      continue;
    }
...
function sortedUnknowns(asset: UniversalSourceFactsV3Asset): UnknownFact[] {
...
function fileFromReceipt(
  file: FoundryUniversalIntakeFile,
  asset: UniversalSourceFactsV3Asset | undefined,
): ReadinessFile {
  if (asset !== undefined) {
    ...
    decisiveNextTests: [
      status === "ambiguous_format"
        ...
        : status === "unclassified_format"
          ...
          : GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V3.decisiveNextTest,
    ],
  });
}
...
function assertExactXbinBinding(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV3, { readonly state: "unavailable" }>,
): void {
...
export interface CompileFoundrySourceReadinessMapV3Input {
 ...
 */
export function compileFoundrySourceReadinessMapV3(
  input: CompileFoundrySourceReadinessMapV3Input,
): FoundrySourceReadinessMapV3 {
  const receipt = FoundryUniversalIntakeReceiptSchema.parse(input.receipt);
  const sourceFacts = FoundryUniversalSourceFactsV3Schema.parse(input.sourceFacts);
  ...
  if (xbinSources.length > 0) {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_SOURCE_READINESS_MAP_V3,
...
export function serializeFoundrySourceReadinessMapV3(
  value: FoundrySourceReadinessMapV3,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundrySourceReadinessMapV3Schema.parse(value)),
  );
}
