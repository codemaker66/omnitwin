...
export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V5 =
  "omnitwin.foundry.operator-evidence-checklist.v5";
export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V5_DIGEST_DOMAIN =
...
function gaussianPlyRequestDefinition(
  ...
  if (unknown === undefined) {
    throw new FoundryIntegrityError(
      ...
      `Source Facts V5 does not export the frozen unknown definition ${code}.`,
    );
  }
...
function mediaRequestDefinition(
  ...
  if (unknown === undefined) {
    throw new FoundryIntegrityError(
      ...
      `Source Facts V5 does not export the frozen media unknown definition ${code}.`,
    );
  }
  return requestDefinition(
    "unknown",
...
function registrationDocumentRequestDefinition(
  ...
  if (unknown === undefined) {
    throw new FoundryIntegrityError(
      ...
      `Source Facts V5 does not export the frozen registration-document unknown definition ${code}.`,
    );
  }
  return requestDefinition(
    "unknown",
...
const REQUEST_DEFINITIONS = {
  ...
  OUTSIDE_SOURCE_FACTS_V5: requestDefinition(
    "gap",
    "bounded_inspection",
    "high",
    "Outside Source Facts V5",
    "The receipt has candidate source families, but Source Facts V5 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: requestDefinition(
    ...
    "Source Facts V5 targeted the file but did not establish its bounded format facts.",
...
const GROUP_DESCRIPTORS = [
  {
    ...
    meaning:
      "Resolve format ambiguity, missing format identification, unsupported V5 coverage, or a bounded inspection failure before relying on deeper source facts.",
  },
  {
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V5),
...
export const FoundryOperatorEvidenceChecklistV5Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundryOperatorEvidenceChecklistV5 = z.infer<
  typeof FoundryOperatorEvidenceChecklistV5Schema
>;
...
function issueArtifact(
  ...
  return FoundryOperatorEvidenceChecklistV5Schema.parse({
    ...payload,
...
export interface CompileFoundryOperatorEvidenceChecklistV5Input {
  readonly readiness: unknown;
}

/**
 * Builds a deterministic, unperformed evidence-request checklist from only an
 * exact Source Readiness Map V5. It performs no evidence acquisition or work.
 */
export function compileFoundryOperatorEvidenceChecklistV5(
  input: CompileFoundryOperatorEvidenceChecklistV5Input,
): FoundryOperatorEvidenceChecklistV5 {
  const readiness = FoundrySourceReadinessMapV5Schema.parse(input.readiness);
  if (readiness.state === "blocked") {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V5,
...
export interface VerifyFoundryOperatorEvidenceChecklistV5Input {
  readonly readiness: unknown;
  readonly checklist: unknown;
}

export function verifyFoundryOperatorEvidenceChecklistV5(
  input: VerifyFoundryOperatorEvidenceChecklistV5Input,
): FoundryOperatorEvidenceChecklistV5 {
  const actual = FoundryOperatorEvidenceChecklistV5Schema.parse(
    input.checklist,
  );
  const expected = compileFoundryOperatorEvidenceChecklistV5({
    readiness: input.readiness,
  });
  if (
    serializeFoundryOperatorEvidenceChecklistV5(actual) !==
    serializeFoundryOperatorEvidenceChecklistV5(expected)
  ) {
    throw new FoundryIntegrityError(
...
export function serializeFoundryOperatorEvidenceChecklistV5(
  value: FoundryOperatorEvidenceChecklistV5,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundryOperatorEvidenceChecklistV5Schema.parse(value)),
  );
}
