...
export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V1 =
  "omnitwin.foundry.operator-evidence-checklist.v1";
export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_DIGEST_DOMAIN =
...
export const FOUNDRY_OPERATOR_EVIDENCE_GAP_CODES = [
  "AMBIGUOUS_FORMAT",
  "NO_SOURCE_OBSERVED",
...
export const FOUNDRY_OPERATOR_EVIDENCE_UNKNOWN_CODES = [
...
> = {
  ...
  registration_input: {
    ...
    criteria: [
      "A reviewed coordinate-frame or control record names every affected relative path and exact SHA-256.",
      "The record states frame names, transform direction and convention, units, control origin, and every declared transform dependency.",
      "Accuracy or registration quality remains unresolved unless a separate residual evaluation against independent control is supplied.",
    ],
    limits:
      "A frame or control record would supply spatial context only; it would not prove transform correctness, accuracy, rights, admission, or permission to run.",
  },
  independent_control: {
...
const REQUEST_DEFINITIONS = {
  ...
  OUTSIDE_SOURCE_FACTS_V1: requestDefinition(
    "gap",
    "bounded_inspection",
    "high",
    "Outside Source Facts V1",
    "The receipt has candidate source families, but Source Facts V1 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: requestDefinition(
    ...
    "Source Facts V1 targeted the file but did not establish its bounded format facts.",
...
const GROUP_DESCRIPTORS = [
  {
    ...
    meaning:
      "Resolve format ambiguity, missing format identification, unsupported V1 coverage, or a bounded inspection failure before relying on deeper source facts.",
  },
  {
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V1),
...
export const FoundryOperatorEvidenceChecklistV1Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundryOperatorEvidenceChecklistV1 = z.infer<
  typeof FoundryOperatorEvidenceChecklistV1Schema
>;
...
): FoundryOperatorEvidenceChecklistV1 {
  const candidate = {
    ...payload,
    checklistSha256: "0".repeat(64),
  } as ArtifactWithoutValidation;
  return FoundryOperatorEvidenceChecklistV1Schema.parse({
    ...payload,
...
export interface CompileFoundryOperatorEvidenceChecklistV1Input {
  readonly readiness: unknown;
}

/**
 * Builds a deterministic, unperformed evidence-request checklist from only an
 * exact Source Readiness Map V1. It performs no evidence acquisition or work.
 */
export function compileFoundryOperatorEvidenceChecklistV1(
  input: CompileFoundryOperatorEvidenceChecklistV1Input,
): FoundryOperatorEvidenceChecklistV1 {
  const readiness = FoundrySourceReadinessMapV1Schema.parse(input.readiness);
  if (readiness.state === "blocked") {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V1,
...
export interface VerifyFoundryOperatorEvidenceChecklistV1Input {
  readonly readiness: unknown;
  readonly checklist: unknown;
}

export function verifyFoundryOperatorEvidenceChecklistV1(
  input: VerifyFoundryOperatorEvidenceChecklistV1Input,
): FoundryOperatorEvidenceChecklistV1 {
  const actual = FoundryOperatorEvidenceChecklistV1Schema.parse(
    input.checklist,
  );
  const expected = compileFoundryOperatorEvidenceChecklistV1({
    readiness: input.readiness,
  });
  if (
    serializeFoundryOperatorEvidenceChecklistV1(actual) !==
    serializeFoundryOperatorEvidenceChecklistV1(expected)
  ) {
    throw new FoundryIntegrityError(
...
export function serializeFoundryOperatorEvidenceChecklistV1(
  value: FoundryOperatorEvidenceChecklistV1,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundryOperatorEvidenceChecklistV1Schema.parse(value)),
  );
}
