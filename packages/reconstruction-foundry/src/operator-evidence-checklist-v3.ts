...
import {
  ...
  type FoundrySourceReadinessMapV3,
} from "./source-readiness-v3.js";

export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V3 =
  "omnitwin.foundry.operator-evidence-checklist.v3";
export const FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V3_DIGEST_DOMAIN =
  "VENVIEWER_FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V3";
...
function gaussianPlyRequestDefinition(
  ...
  if (unknown === undefined) {
    throw new FoundryIntegrityError(
      ...
      `Source Facts V3 does not export the frozen unknown definition ${code}.`,
    );
  }
...
const REQUEST_DEFINITIONS = {
  ...
  OUTSIDE_SOURCE_FACTS_V3: requestDefinition(
    "gap",
    "bounded_inspection",
    "high",
    "Outside Source Facts V3",
    "The receipt has candidate source families, but Source Facts V3 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: requestDefinition(
    ...
    "Source Facts V3 targeted the file but did not establish its bounded format facts.",
...
const GROUP_DESCRIPTORS = [
  {
    ...
    meaning:
      "Resolve format ambiguity, missing format identification, unsupported V3 coverage, or a bounded inspection failure before relying on deeper source facts.",
  },
  {
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V3),
...
export const FoundryOperatorEvidenceChecklistV3Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundryOperatorEvidenceChecklistV3 = z.infer<
  typeof FoundryOperatorEvidenceChecklistV3Schema
>;
...
function issueArtifact(
  ...
  return FoundryOperatorEvidenceChecklistV3Schema.parse({
    ...payload,
...
export interface CompileFoundryOperatorEvidenceChecklistV3Input {
  readonly readiness: unknown;
}

/**
 * Builds a deterministic, unperformed evidence-request checklist from only an
 * exact Source Readiness Map V3. It performs no evidence acquisition or work.
 */
export function compileFoundryOperatorEvidenceChecklistV3(
  input: CompileFoundryOperatorEvidenceChecklistV3Input,
): FoundryOperatorEvidenceChecklistV3 {
  const readiness = FoundrySourceReadinessMapV3Schema.parse(input.readiness);
  if (readiness.state === "blocked") {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_OPERATOR_EVIDENCE_CHECKLIST_V3,
...
export interface VerifyFoundryOperatorEvidenceChecklistV3Input {
  readonly readiness: unknown;
  readonly checklist: unknown;
}

export function verifyFoundryOperatorEvidenceChecklistV3(
  input: VerifyFoundryOperatorEvidenceChecklistV3Input,
): FoundryOperatorEvidenceChecklistV3 {
  const actual = FoundryOperatorEvidenceChecklistV3Schema.parse(
    input.checklist,
  );
  const expected = compileFoundryOperatorEvidenceChecklistV3({
    readiness: input.readiness,
  });
  if (
    serializeFoundryOperatorEvidenceChecklistV3(actual) !==
    serializeFoundryOperatorEvidenceChecklistV3(expected)
  ) {
    throw new FoundryIntegrityError(
...
export function serializeFoundryOperatorEvidenceChecklistV3(
  value: FoundryOperatorEvidenceChecklistV3,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundryOperatorEvidenceChecklistV3Schema.parse(value)),
  );
}
