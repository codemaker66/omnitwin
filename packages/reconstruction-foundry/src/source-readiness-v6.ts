...
import {
  ...
  type FoundryPlyPointCloudSourceFactsFailureCode,
...
import {
  ...
  type UniversalSourceFactsV6Asset,
...
export const FOUNDRY_SOURCE_READINESS_MAP_V6 =
  "omnitwin.foundry.source-readiness-map.v6";
export const FOUNDRY_SOURCE_READINESS_MAP_V6_DIGEST_DOMAIN =
  "VENVIEWER_FOUNDRY_SOURCE_READINESS_MAP_V6";
...
const CANONICAL_POINT_PLY_UNKNOWNS = Object.freeze(
  [...FOUNDRY_POINT_PLY_UNKNOWNS]
...
const LaneCountsSchema = z
  .object({
    ...
    outsideSourceFactsV6Count: z.number().int().nonnegative(),
...
const BlockedReasonSchema = z
  .object({
    ...
    message: z.literal(
      "The source set includes an XGRIDS XBIN candidate, so Source Readiness Map V6 exposes no partial file or lane evidence.",
    ),
...
const ArtifactBaseSchema = z
  .object({
    schemaVersion: z.literal(FOUNDRY_SOURCE_READINESS_MAP_V6),
...
export const FoundrySourceReadinessMapV6Schema = z
  .discriminatedUnion("state", [AvailableArtifactSchema, BlockedArtifactSchema])
  .superRefine(validateArtifact);
export type FoundrySourceReadinessMapV6 = z.infer<
  typeof FoundrySourceReadinessMapV6Schema
>;
...
const GENERIC_GAP_DETAILS = {
  ...
  OUTSIDE_SOURCE_FACTS_V6: {
    label: "Outside Source Facts V6",
    reason: "The receipt has candidate source families, but Source Facts V6 does not inspect this file format.",
  ...
  SOURCE_FACTS_NOT_ESTABLISHED: {
    ...
    reason: "Source Facts V6 targeted the file but did not establish its bounded format facts.",
...
function validateFileEvidenceState(
  ...
  if (factsStatus) {
    ...
    const formatMatches =
      ...
      (file.format === "gaussian_ply" && file.inputType === "gaussian_ply") ||
      (file.format === "ply" && file.inputType === "ply_point_cloud") ||
      ((file.format === "jpeg" || file.format === "png") &&
        file.inputType === "generic_image") ||
      (file.format === "iso_bmff" && file.inputType === "video") ||
      (file.format === "media_container" &&
        (file.inputType === "generic_image" ||
          file.inputType === "video" ||
          file.inputType === "drone_media")) ||
      ((file.format === "csv" || file.format === "json" ||
        file.format === "calibration_trajectory_document") &&
        (file.inputType === "calibration_bundle" ||
          file.inputType === "trajectory")) ||
    ...
    const isPlaceholderMediaFormat = file.format === "media_container";
    const isEstablishedMediaFormat =
      file.format === "jpeg" ||
      file.format === "png" ||
      file.format === "iso_bmff";
    if (
      (isPlaceholderMediaFormat && file.status !== "facts_not_established") ||
      (isEstablishedMediaFormat && file.status !== "facts_established")
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["format"],
    ...
    const isPlaceholderRegistrationDocument =
      file.format === "calibration_trajectory_document";
    const isEstablishedRegistrationDocument =
      file.format === "csv" || file.format === "json";
    if (
      (isPlaceholderRegistrationDocument && file.status !== "facts_not_established") ||
      (isEstablishedRegistrationDocument && file.status !== "facts_established")
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["format"],
        message:
          "calibration_trajectory_document is reserved for facts-not-established while CSV and JSON require established complete-document facts",
      });
    }
    if (file.format === "ply" && file.inputType === "ply_point_cloud") {
      const inspection = file.inspection;
      if (
        JSON.stringify(file.unknowns) !==
          JSON.stringify(CANONICAL_POINT_PLY_UNKNOWNS)
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["unknowns"],
          message: "point PLY readiness must preserve the complete frozen V6 unknown set",
        });
      }
      if (file.status === "facts_established") {
        if (
          inspection?.state !== "established" ||
          inspection.category !== "established" ||
          ...
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["inspection"],
            message: "established point PLY readiness requires the frozen success inspection profile",
          });
        }
      } else {
        const code = inspection?.code ?? "";
        const isFailureCode = (
          FOUNDRY_POINT_PLY_SOURCE_FACTS_FAILURE_CODES as readonly string[]
        ).includes(code);
        const expectedCategory = isFailureCode
          ? FOUNDRY_POINT_PLY_SOURCE_FACTS_FAILURE_CATEGORY_BY_CODE[
              code as FoundryPlyPointCloudSourceFactsFailureCode
            ]
          : undefined;
        if (
          ...
          inspection.category !== expectedCategory ||
          inspection.coverage !== "none"
        ) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["inspection"],
            message: "failed point PLY readiness must match the frozen non-cancelled failure registry",
          });
        }
      }
    }
    const detectionCarriesInputType = file.inputType !== null &&
      file.detection.candidates.some(
        (candidate) => candidate.inputType === file.inputType,
      );
    const establishedGaussianRefinement =
      file.inputType === "gaussian_ply" &&
      file.format === "gaussian_ply" &&
      file.inspection?.state === "established" &&
      file.detection.candidates.some(
        (candidate) => candidate.inputType === "ply_point_cloud",
      );
    const establishedMediaRefinement =
      file.inspection?.state === "established" &&
      ((file.inputType === "generic_image" &&
        (file.format === "jpeg" || file.format === "png")) ||
        (file.inputType === "video" && file.format === "iso_bmff")) &&
      file.detection.candidates.some((candidate) =>
        MEDIA_RECEIPT_CANDIDATE_TYPES.includes(
          candidate.inputType as (typeof MEDIA_RECEIPT_CANDIDATE_TYPES)[number],
        ),
      );
    if (
      file.inputType !== null &&
      !detectionCarriesInputType &&
      !establishedGaussianRefinement &&
      !establishedMediaRefinement
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["detection", "candidates"],
        message: "Source Facts input type must remain present in receipt detection evidence or be an established byte-supported Gaussian or media-container refinement",
      });
    }
  ...
  if (
    ...
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["status"],
      message: "a point PLY receipt candidate must retain its inherited or V6 facts result",
    });
  }
  if (file.unknowns.length > 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["unknowns"],
      message: "files outside Source Facts V6 cannot claim format unknowns",
    });
  }
...
function statusDecisiveTest(status: FileStatus): string | null {
  switch (status) {
    ...
    case "outside_source_facts_v6":
      return GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V6.decisiveNextTest;
...
function countsForFiles(files: readonly ReadinessFile[]): z.infer<typeof LaneCountsSchema> {
  return {
    ...
    outsideSourceFactsV6Count: files.filter(
      (file) => file.status === "outside_source_facts_v6",
    ).length,
...
function summaryForAvailable(
  ...
  return {
    ...
    outsideSourceFactsV6Count: counts.outsideSourceFactsV6Count,
...
function issueArtifact(
  ...
  return FoundrySourceReadinessMapV6Schema.parse(issued);
}
...
const MEDIA_RECEIPT_CANDIDATE_TYPES =
  FOUNDRY_MEDIA_CONTAINER_RECEIPT_CANDIDATE_INPUT_TYPES;
...
function isMediaFormat(value: string): value is MediaFormat {
  return MEDIA_FORMATS.includes(value as MediaFormat);
}

function establishedMediaFormatFromReceiptMagic(
  file: FoundryUniversalIntakeFile,
): Exclude<MediaFormat, "media_container"> | null {
  const magic = file.inspection.magicHex;
  if (magic.startsWith("89504e470d0a1a0a")) return "png";
  if (magic.startsWith("ffd8")) return "jpeg";
  if (magic.length >= 16 && magic.slice(8, 16) === "66747970") {
    return "iso_bmff";
  }
  return null;
}

function mediaReceiptCandidateTypes(
  file: FoundryUniversalIntakeFile,
): Array<(typeof MEDIA_RECEIPT_CANDIDATE_TYPES)[number]> {
  const present = new Set(
    file.detection.candidates.map((candidate) => candidate.inputType),
  );
  return MEDIA_RECEIPT_CANDIDATE_TYPES.filter((inputType) =>
...
function claimedByV1ThroughV3SourceFacts(
  ...
  const magic = file.inspection.magicHex;
  if (
    magic.startsWith("4153544d2d453537") ||
    magic.startsWith("676c5446")
  ) return true;
  const suffix = extension(file.path);
  return hasCandidate(file, "gaussian_ply") ||
    hasCandidate(file, "ply_point_cloud") ||
    hasCandidate(file, "spz") || suffix === ".spz" ||
    hasCandidate(file, "sog") || suffix === ".sog" ||
    hasCandidate(file, "generic_e57") ||
    hasCandidate(file, "matterport_e57") || suffix === ".e57" ||
    hasCandidate(file, "glb_gltf") || suffix === ".gltf" || suffix === ".glb" ||
    hasCandidate(file, "obj") || suffix === ".obj";
}

function expectedTarget(
  file: FoundryUniversalIntakeFile,
  refinedAsset?: UniversalSourceFactsV6Asset,
): ExpectedTarget | null {
  const magic = file.inspection.magicHex;
  ...
  const establishedGaussianRefinement = hasCandidate(file, "ply_point_cloud") &&
    refinedAsset?.format === "gaussian_ply" &&
    refinedAsset.inspection.state === "established";
  if (hasCandidate(file, "gaussian_ply") || establishedGaussianRefinement) {
    return { inputType: "gaussian_ply", format: "gaussian_ply" };
  }
  if (hasCandidate(file, "ply_point_cloud")) {
    return { inputType: "ply_point_cloud", format: "ply" };
  }
  ...
  if (claimedByV1ThroughV3SourceFacts(file)) return null;
  if (
    file.detection.status === "detected" &&
    file.detection.candidates.length === 1 &&
    (file.detection.candidates[0]?.inputType === "calibration_bundle" ||
      file.detection.candidates[0]?.inputType === "trajectory")
  ) {
    const inputType = file.detection.candidates[0].inputType;
    const suffix = extension(file.path);
    const format = refinedAsset?.inspection.state === "facts_not_established"
      ? "calibration_trajectory_document"
      : suffix === ".csv"
        ? "csv"
        : suffix === ".json"
  ...
  const mediaCandidates = mediaReceiptCandidateTypes(file);
  if (mediaCandidates.length > 0) {
    const format = refinedAsset?.inspection.state === "facts_not_established"
      ? "media_container"
      : establishedMediaFormatFromReceiptMagic(file) ?? "media_container";
...
function assetIdentity(asset: UniversalSourceFactsV6Asset): {
...
function assertAvailableSourceFactsCoverage(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV6, { readonly state: "available" }>,
): Map<string, UniversalSourceFactsV6Asset> {
  ...
  for (const file of receipt.files) {
    ...
    const target = expectedTarget(file, asset);
    if (target === null) {
      if (asset !== undefined) {
        throw new FoundryIntegrityError(
          "SOURCE_READINESS_UNEXPECTED_FACTS_TARGET",
          `Source Facts targets ${file.path}, which is outside its V6 target rules.`,
        );
      }
      continue;
    }
    ...
    if (isMediaFormat(asset.format)) {
      const expectedCandidates = mediaReceiptCandidateTypes(file);
      if (
        !("receiptCandidateInputTypes" in asset.source) ||
        JSON.stringify(asset.source.receiptCandidateInputTypes) !==
          JSON.stringify(expectedCandidates)
      ) {
        throw new FoundryIntegrityError(
          "SOURCE_READINESS_MEDIA_CANDIDATE_BINDING_MISMATCH",
          `Source Facts media candidates do not exactly match receipt evidence for ${file.path}.`,
        );
      }
    }
    if (
      asset.source.inputType === "calibration_bundle" ||
      asset.source.inputType === "trajectory"
    ) {
      if (
        !("receiptCandidateInputTypes" in asset.source) ||
        JSON.stringify(asset.source.receiptCandidateInputTypes) !==
          JSON.stringify([target.inputType])
      ) {
        throw new FoundryIntegrityError(
          "SOURCE_READINESS_CALIBRATION_TRAJECTORY_CANDIDATE_BINDING_MISMATCH",
          `Source Facts registration-document candidate does not exactly match receipt evidence for ${file.path}.`,
        );
      }
    }
  }
...
function sortedUnknowns(asset: UniversalSourceFactsV6Asset): UnknownFact[] {
...
function fileFromReceipt(
  file: FoundryUniversalIntakeFile,
  asset: UniversalSourceFactsV6Asset | undefined,
): ReadinessFile {
  if (asset !== undefined) {
    ...
    decisiveNextTests: [
      status === "ambiguous_format"
        ...
        : status === "unclassified_format"
          ...
          : GENERIC_GAP_DETAILS.OUTSIDE_SOURCE_FACTS_V6.decisiveNextTest,
    ],
  });
}
...
function assertExactXbinBinding(
  ...
  sourceFacts: Extract<FoundryUniversalSourceFactsV6, { readonly state: "unavailable" }>,
): void {
...
export interface CompileFoundrySourceReadinessMapV6Input {
 ...
 */
export function compileFoundrySourceReadinessMapV6(
  input: CompileFoundrySourceReadinessMapV6Input,
): FoundrySourceReadinessMapV6 {
  const receipt = FoundryUniversalIntakeReceiptSchema.parse(input.receipt);
  const sourceFacts = FoundryUniversalSourceFactsV6Schema.parse(input.sourceFacts);
  ...
  if (xbinSources.length > 0) {
    ...
    return issueArtifact({
      schemaVersion: FOUNDRY_SOURCE_READINESS_MAP_V6,
...
export function serializeFoundrySourceReadinessMapV6(
  value: FoundrySourceReadinessMapV6,
): string {
  return stableCanonicalJson(
    toCanonicalJson(FoundrySourceReadinessMapV6Schema.parse(value)),
  );
}
