...
**Founder correction, 2026-09-06:** the old OBJ floor and panorama-wall recollections are comparison clues, not instructions to choose those representations. [Direction 17](../docs/plan/17-GRAND-HALL-RECONSTRUCTION-DIRECTION-2026-09-06.md) governs the multi-source reconstruction programme and current ownership. It supersedes the older fixed floor/appearance recipes below; plan 14 ROUND TWO still governs its specific training experiments. No source viewer, renderer agreement score or empty-scene 4090 measurement closes the product goal.

## The /goal block

Make the captured rooms spectacular and fast: the Grand Hall walk beautiful and sharp at one quality bar, 60 fps in motion with the same accepted appearance on the RTX 4090 laptop, on ordinary office computers, and on current iPhones and iPads including the entry models (plan 16 §4), first view in seconds; then the other seven rooms on the same recipe. Follow docs/plan/17-GRAND-HALL-RECONSTRUCTION-DIRECTION-2026-09-06.md: finish the valid court with its current owner, jointly calibrate the useful observations in a fixed metric frame, compare deterministic and learned multi-source reconstruction against the strongest individual source, and integrate the winning result into the real furnished planner. The OBJ floor and remembered panorama walls are comparison clues, not prescribed final assets. Plan 14 ROUND TWO governs its specific import/training experiments; do not execute superseded W-card recipes. Add the device matrix with fixtures chosen by engineering and measured on physical devices, the R2 custom domain, and plan 16's runtime experiments, including a same-source comparison of Spark, equivalent-work optimisations and a WebGPU prototype. A visibly reduced mobile mode cannot close this goal. PSNR 50+ is tracked under the distinct declared protocols and every report says which number it reports.
...
The floor needs improved reconstruction evidence; the OBJ is a baseline, not the selected solution. Every XGRIDS level is the whole room at one density; serve one level plus the sky, never a stack; the coarsest level is an interim first view. A Spark mesh cannot load hidden. One SparkRenderer host per scene. PSNR 50+ is tracked under the distinct protocols in D6. Once calibrated, either PortalCam capture and the panorama observations may contribute to the source-selection, confidence-blending or learned reconstruction experiments in direction 17. Panorama pinhole views preserve their parent rig. The served Gaussian floor slab is the current runtime datum; accepted measured geometry and an explicit transform govern a future reconstructed floor.
...
Follow direction 17's current sequence with ownership checked on the execution board: complete the fixed-view court, exclusions and registration evidence; run the architectural-bay A/B/C/D comparisons using metric geometry and calibrated appearance; inspect the result in stills and motion; integrate the accepted representation into the planner alongside its existing furniture and measurement systems; measure delivery and sustained device performance; then extend the proven recipe to the remaining hall and other rooms. Acceptance of registration requires unused observed correspondences and metric stability, not an ICP threshold alone. The former fixed 5 mm floor orthomosaic is an optional comparison candidate, not the mandated reconstruction. Goal 08 owns the specific capped training experiments under plan 14 ROUND TWO.
